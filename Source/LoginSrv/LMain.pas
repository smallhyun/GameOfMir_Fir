...
uses
  ...
  Controls, Forms, Dialogs, StdCtrls, ExtCtrls, D7ScktComp, SyncObjs, Grids,
  Buttons, IniFiles, MudUtil, Parse, Menus, Grobal2, LSShare, SDK, Common,
  AppEvnts, DateUtils;
type
    ...
    while (True) do begin
      ...
      if sMsg <> '' then begin
        ...
        case Code of
          ...
          'S': begin
              if CompareLStr(sMsg, '127.0.0.', Length('127.0.0.')) then begin
                GateInfo.sIPaddr := GetGatePublicAddr(Config, sMsg);
                //MainOutMessage(GateInfo.sIPaddr + ' 登录网关连接');
              end;
...
begin
  ...
  try
    for I := Config.SessionList.Count - 1 downto 0 do begin
      ...
      if ConnInfo.boKicked then begin
        Dispose(ConnInfo);
        Config.SessionList.Delete(I);
      end;
    end;
...
begin
  if (UserInfo.sAccount = '') then begin
    ...
    DefMsg := MakeDefaultMsg(SM_SERVERNAME, Year, Month, Day, Config.ServerNameList.Count);
