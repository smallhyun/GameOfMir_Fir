...
type
  ...
  public
    ...
    procedure AddEvent(Event: TEvent; NeedRun: Boolean = True);
...
begin
 ...
 { m_ClosedEventList.Lock;
  ...
  {finally
    ...
    MainOutMessage('[Exception] TEventManager::Run Error:' + IntToStr(nError));
    if (nError = 4) then
      MainOutMessage('[Exception] TEventManager::Run m_nServerEventType:' + IntToStr(Event.m_nServerEventType) + ' m_nEventType:' + IntToStr(Event.m_nEventType));
  end;
end;
...
begin
  ...
  {if (nType = ET_STONEMINE) then begin
    m_NotRunEventList.Lock;
    try
      for I := 0 to m_NotRunEventList.Count - 1 do begin
        Event := TEvent(m_NotRunEventList.Items[I]);
        if Event.m_boVisible and (not Event.m_boClosed) and (Event.m_OwnActorObject = OwnActorObject) and
          (abs(Event.m_nX - nX) <= nRange) and
          (abs(Event.m_nY - nY) <= nRange) and
          (Event.m_nEventType = nType) then begin
...
begin
  ...
  {if (Result = nil) and (nType = ET_STONEMINE) then begin
    m_NotRunEventList.Lock;
    try
      for I := 0 to m_NotRunEventList.Count - 1 do begin
        Event := TEvent(m_NotRunEventList.Items[I]);
...
procedure TEventManager.AddEvent(Event: TEvent; NeedRun: Boolean);
begin
  if NeedRun then begin
    m_EventList.Lock;
    try
...
begin
  ...
  m_NotRunEventList := TGList.Create;
...
begin
  ...
  for I := 0 to m_NotRunEventList.Count - 1 do begin
    TEvent(m_NotRunEventList.Items[I]).Free;
  end;
  m_NotRunEventList.Free;
    ...
    if m_Envir <> nil then begin
      ...
      for I := 0 to ActorObjectList.Count - 1 do begin
        ...
        if (TargeTActorObject <> nil) and (m_OwnActorObject <> nil) and (m_OwnActorObject.IsProperTarget(TargeTActorObject)) then begin
          ...
          TargeTActorObject.SendMsg(m_OwnActorObject, RM_MAGSTRUCK_MINE, 0, nPowerValue, 0, 0, '');
        end;
      end;
    end;
    ActorObjectList.Free;
