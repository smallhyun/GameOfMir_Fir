...
begin
  try
    ...
    if FileExists(sScriptFile) and (Map <> nil) then begin
      ...
      g_FunctionNPC.m_UnAllowLableList.Add('@UsePlugin');
    ...
    for I := 0 to tMerchantList.Count - 1 do begin
      ...
      if (sLineText <> '') and (sLineText[1] <> ';') then begin
        ...
        if (sScript <> '') and (sMapName <> '') and (sAppr <> '') then begin
          ...
          case n100 of
            2000..2999: begin
                tMerchantNPC.m_nVarIdx := n100 - 2000;
...
var
  ...
  begin
    ...
    if sCmd = sSC_CHECKMAPAIHUMANCOUNT then begin
      nCMDCode := nSC_CHECKMAPAIHUMANCOUNT;
      goto L001;
    end;
    if sCmd = sSC_CHECKITEMDURA then begin
      nCMDCode := nSC_CHECKITEMDURA;
      goto L001;
    end;
    if sCmd = sSC_CHECKITEMCOLOR then begin
      nCMDCode := nSC_CHECKITEMCOLOR;
      goto L001;
    end;
    if nCMDCode <= 0 then begin
      if Assigned(PlugInEngine.QuestConditionScriptCmd) then begin
        nCMDCode := PlugInEngine.QuestConditionScriptCmd(PAnsiChar(sCmd));
  ...
  begin
    ...
    if sCmd = sSC_UNBINDUSEITEM then begin
      nCMDCode := nSC_UNBINDUSEITEM;
      goto L001;
    end;
    if sCmd = sSC_STARTMOVEITEMVALUE then begin
      nCMDCode := nSC_STARTMOVEITEMVALUE;
      goto L001;
    end;
    if sCmd = sSC_STOPMOVEITEMVALUE then begin
      nCMDCode := nSC_STOPMOVEITEMVALUE;
      goto L001;
    end;
    if sCmd = sSC_MOVEITEMVALUE then begin
      nCMDCode := nSC_MOVEITEMVALUE;
      goto L001;
    end;
    if sCmd = sSC_MOVEITEMVALUEFINISH then begin
      nCMDCode := nSC_MOVEITEMVALUEFINISH;
      goto L001;
    end;
    if sCmd = sSC_SETMOVEITEMVALUE then begin
      nCMDCode := nSC_SETMOVEITEMVALUE;
      goto L001;
    end;

    if sCmd = sSC_SETACTIVEATTACKHUM then begin
      nCMDCode := nSC_SETACTIVEATTACKHUM;
      goto L001;
    end;
    if sCmd = sSC_SETCHECKUSEPLUGIN then begin
      nCMDCode := nSC_SETCHECKUSEPLUGIN;
      goto L001;
    end;
    ...
    if sCmd = sSC_CHANGESPEED then begin
      nCMDCode := nSC_CHANGESPEED;
      goto L001;
    end;
    if sCmd = sSC_PLAYEFFECT then begin
      nCMDCode := nSC_PLAYEFFECT;
      goto L001;
    end;
    if sCmd = sSC_SCREENEFFECT then begin
      nCMDCode := nSC_SCREENEFFECT;
      goto L001;
    end;

    if sCmd = sSC_RECALLSELF then begin
      nCMDCode := nSC_RECALLSELF;
      goto L001;
    end;

    if sCmd = sSC_COPYHUMUSEITEMS then begin
      nCMDCode := nSC_COPYHUMUSEITEMS;
      goto L001;
    end;
    if sCmd = sSC_RANGEGOTO then begin
      nCMDCode := nSC_RANGEGOTO;
      goto L001;
    end;
    ...
    if sCmd = sSC_CHANGEITEMCOLOR then begin
      nCMDCode := nSC_CHANGEITEMCOLOR;
      goto L001;
    end;
    if nCMDCode <= 0 then begin
      if Assigned(PlugInEngine.QuestActionScriptCmd) then begin
        nCMDCode := PlugInEngine.QuestActionScriptCmd(PAnsiChar(sCmd));
        if nCMDCode >= 0 then goto L001;
      end;
    end;
