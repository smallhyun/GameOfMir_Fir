        ...
        if (BaseObject <> nil) and (BaseObject.m_ObjType = t_Actor) then begin
          ...
          if ActorObject <> nil then begin
            if not ActorObject.m_boGhost
              ...
              and not ActorObject.m_boObMode
              and (ActorObject.m_dwChangeModeExTick[1] <= 0) then begin
              Result := False;
              Break;
            end;
          end;
...
begin
  Result := nil;
  if TActorObject(ActorObject).m_btRaceServer = RC_PLAYOBJECT then begin
    for I := 0 to m_QuestList.Count - 1 do begin
      MapQuestFlag := m_QuestList.Items[I];
      nFlagValue := TPlayObject(ActorObject).GetQuestFalgStatus(MapQuestFlag.nFlag);
