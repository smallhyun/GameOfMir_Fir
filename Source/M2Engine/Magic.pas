  ...
  if BaseObject.IsProperTargetSKILL_57(TargeTActorObject) then begin
    if (Random(5 + UserMagic.btLevel) + UserMagic.btLevel) >= 5 then begin
      ...
      if g_FunctionNPC <> nil then begin
        ...
        g_FunctionNPC.GotoLable(PlayObject, '@Revival', False);
      end;
      Result := True;
    end;
  end;
...
begin
  ...
  nPower := Round(nPower * (g_Config.nSkill63Rate / 100));
  ...
  for I := 0 to BaseObjectList.Count - 1 do begin
    ...
    if PlayObject.IsProperTarget(BaseObject) then begin
      ...
      BaseObject.m_boNotDefendoof := PlayObject.GetNotDefendoof; //忽视目标防御
      nPowerValue := nPower + PlayObject.GetAddPowerPoint(5, nPower);
      ...
      nPowerValue := (GetPower13(40, UserMagic) + GetRPow(PlayObject.m_WAbil.SC) * 2);
...
begin
  ...
  if PlayObject.IsProperTarget(TargeTActorObject) then begin
    ...
    if g_Config.boHeroMag64DownHealth then begin
  ...
  case UserMagic.MagicInfo.wMagicId of //
    ...
    SKILL_52 {52}: begin //诅咒术
        ...
        if CheckAmulet(PlayObject, 1, 3, nAmuletIdx) then begin
          ...
          nPower := Random(10 + UserMagic.btLevel) + UserMagic.btLevel;
    ...
    SKILL_SINSU {30}: begin
        if g_Config.boAllowRecallAll or g_Config.boAllowRecallDogz then begin
          boSpellFail := True;
    ...
    SKILL_59: begin
       //MainOutMessage('g_Config.boAllowRecallMoon '+BooleanToStr(g_Config.boAllowRecallMoon));
        if g_Config.boAllowRecallAll or g_Config.boAllowRecallMoon then begin
...
begin
  ...
  if TargeTActorObject.m_btRaceServer = RC_PLAYOBJECT then begin
    if g_FunctionNPC <> nil then begin
      TPlayObject(TargeTActorObject).m_nScriptGotoCount := 0;
      g_FunctionNPC.GotoLable(TPlayObject(TargeTActorObject), '@Revival', False);
    end;
  end;
  Result := True;
end;
