...
type
  TfrmFunctionConfig = class(TForm)
    ...
    Label186: TLabel;
    EditAIWhisperSayRate: TSpinEdit;
    Label187: TLabel;
    EditAIStruckSayRate: TSpinEdit;
    GroupBox107: TGroupBox;
    CheckBoxDelDuraZeroItem: TCheckBox;
    TabSheet17: TTabSheet;
    PageControlItemEffect: TPageControl;
    TabSheet19: TTabSheet;
    TabSheet23: TTabSheet;
    TabSheet24: TTabSheet;
    GroupBox108: TGroupBox;
    ListBoxWilNameList: TListBox;
    ButtonWilNameUP: TButton;
    ButtonWilNameDown: TButton;
    Label189: TLabel;
    EditWilName: TEdit;
    ButtonWilAdd: TButton;
    ButtonWilDel: TButton;
    ButtonWilSave: TButton;
    GroupBox109: TGroupBox;
    ListBoxEffectList: TListBox;
    GroupBoxEffectItem: TGroupBox;
    Label191: TLabel;
    Label192: TLabel;
    GroupBox111: TGroupBox;
    lbl5: TLabel;
    lbl7: TLabel;
    EditItemEffectOffset3: TSpinEdit;
    ComboBoxEffectFileIndex3: TComboBox;
    ...
    lbl10: TLabel;
    lbl12: TLabel;
    EditItemEffectOffset2: TSpinEdit;
    EditItemEffectImageCount2: TSpinEdit;
    ComboBoxEffectFileIndex2: TComboBox;
    EditEffectDesc: TEdit;
    EditEffectIndex: TSpinEdit;
    GroupBox110: TGroupBox;
    lbl2: TLabel;
    lbl3: TLabel;
    lbl4: TLabel;
    EditItemEffectOffset1: TSpinEdit;
    EditItemEffectImageCount1: TSpinEdit;
    ComboBoxEffectFileIndex1: TComboBox;
    ButtonEffectSave: TButton;
    GroupBox113: TGroupBox;
    ListBoxStdItemList: TListBox;
    GroupBox114: TGroupBox;
    ListBoxEffectItemList: TListBox;
    Label193: TLabel;
    Label194: TLabel;
    LabelEffectMemo: TLabel;
    ...
    procedure EditAIWhisperSayRateChange(Sender: TObject);
    procedure EditAIStruckSayRateChange(Sender: TObject);
    procedure CheckBoxDelDuraZeroItemClick(Sender: TObject);
    procedure ListBoxEffectListClick(Sender: TObject);
    ...
    procedure ButtonWilNameUPClick(Sender: TObject);
    procedure ButtonWilNameDownClick(Sender: TObject);
    procedure ButtonWilAddClick(Sender: TObject);
    procedure ButtonWilDelClick(Sender: TObject);
    procedure ButtonWilSaveClick(Sender: TObject);
    procedure ListBoxStdItemListKeyDown(Sender: TObject; var Key: Word;
      Shift: TShiftState);
    procedure ListBoxStdItemListClick(Sender: TObject);
    ...
    procedure ComboBoxEffectFileIndex2Change(Sender: TObject);
    procedure ComboBoxEffectFileIndex1Change(Sender: TObject);
    procedure ComboBoxEffectFileIndex3Change(Sender: TObject);
...
begin
  ...
  CheckBoxDelDuraZeroItem.Checked := g_Config.boDelDuraZeroItem;

  EditAIWhisperSayRate.Value := g_Config.nAIWhisperSayRate;
  EditAIStruckSayRate.Value := g_Config.nAIStruckSayRate;


  ListBoxWilNameList.Items.Clear;
  ComboBoxEffectFileIndex1.Items.Clear;
  ComboBoxEffectFileIndex2.Items.Clear;
  ComboBoxEffectFileIndex3.Items.Clear;
  ...
  try
    for I := 0 to UserEngine.StdItemList.Count - 1 do begin
      ...
      ListBoxStdItemList.Items.AddObject(StdItem.Name, TObject(StdItem));
    end;
  finally
    LeaveCriticalSection(ProcessHumanCriticalSection);
  end;
  ListBoxWilNameList.Items.AddStrings(g_EffectImageList);

  ComboBoxEffectFileIndex1.Items.Add('关闭特效');
  ComboBoxEffectFileIndex2.Items.Add('关闭特效');
  ComboBoxEffectFileIndex3.Items.Add('关闭特效');

  ComboBoxEffectFileIndex1.Items.AddStrings(g_EffectImageList);
  ComboBoxEffectFileIndex2.Items.AddStrings(g_EffectImageList);
  ComboBoxEffectFileIndex3.Items.AddStrings(g_EffectImageList);

  for I := 0 to g_EffectItemList.Count - 1 do begin
    if (Integer(g_EffectItemList.Objects[I]) > 0) then begin
      ItemEffect := GetItemEffect(Integer(g_EffectItemList.Objects[I]));
      if ItemEffect <> nil then begin
  ...
  RefEffectList;
  ButtonWilAdd.Enabled := True;
  ButtonWilDel.Enabled := False;
  ButtonWilSave.Enabled := False;

  ButtonEffectAdd.Enabled := True;
  ButtonEffectChg.Enabled := False;
  ButtonEffectDel.Enabled := False;
  ButtonEffectSave.Enabled := False;

  ButtonEffectItemAdd.Enabled := True;
  ...
  PageControlItemEffect.ActivePageIndex := 0;
...
begin
  ...
  Config.WriteBool('Setup', 'DelDuraZeroItem', g_Config.boDelDuraZeroItem);
  uModValue();
end;
...
begin
  ...
  Config.WriteInteger('Setup', 'AIWhisperSayRate', g_Config.nAIWhisperSayRate);
  Config.WriteInteger('Setup', 'AIStruckSayRate', g_Config.nAIStruckSayRate);
...
procedure TfrmFunctionConfig.EditAIWhisperSayRateChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nAIWhisperSayRate := EditAIWhisperSayRate.Value;
  ModValue();
end;

procedure TfrmFunctionConfig.EditAIStruckSayRateChange(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.nAIStruckSayRate := EditAIStruckSayRate.Value;
  ModValue();
end;

procedure TfrmFunctionConfig.CheckBoxDelDuraZeroItemClick(Sender: TObject);
begin
  if not boOpened then Exit;
  g_Config.boDelDuraZeroItem := CheckBoxDelDuraZeroItem.Checked;
  ModValue();
end;


procedure TfrmFunctionConfig.RefEffectList();
var
  I, nIndex, nIndex1: Integer;
  ItemEffect: pTItemEffect;
begin
  nIndex := ListBoxEffectList.ItemIndex;
  nIndex1 := ComboBoxEffectIndex.ItemIndex;
  ListBoxEffectList.Items.Clear;
  ComboBoxEffectIndex.Items.Clear;

  for I := 0 to g_EffectList.Count - 1 do begin
    ItemEffect := g_EffectList.Items[I];
    ListBoxEffectList.Items.AddObject('特效编号:' + IntToStr(ItemEffect.Index), TObject(ItemEffect));
  ...
  if (nIndex >= 0) and (nIndex < ListBoxEffectItemList.Items.Count) then
    ListBoxEffectList.ItemIndex := nIndex;

  if (nIndex1 >= 0) and (nIndex < ComboBoxEffectIndex.Items.Count) then
    ComboBoxEffectIndex.ItemIndex := nIndex1;
end;

procedure TfrmFunctionConfig.ListBoxEffectListClick(Sender: TObject);
var
  nIndex: Integer;
  ItemEffect: pTItemEffect;
begin
  nIndex := ListBoxEffectList.ItemIndex;
  if nIndex >= 0 then begin
    ItemEffect := pTItemEffect(ListBoxEffectList.Items.Objects[nIndex]);
    EditEffectIndex.Value := ItemEffect.Index;
    GroupBoxEffectItem.Caption := '特效编号:' + IntToStr(ItemEffect.Index);
    EditEffectDesc.Text := ItemEffect.EffectDesc;
    EditItemEffectOffset1.Value := ItemEffect.Offset1;
    EditItemEffectOffset2.Value := ItemEffect.Offset2;
    EditItemEffectOffset3.Value := ItemEffect.Offset3;

    ComboBoxEffectFileIndex1.ItemIndex := ItemEffect.FileIndex1 + 1;
    ComboBoxEffectFileIndex2.ItemIndex := ItemEffect.FileIndex2 + 1;
    ComboBoxEffectFileIndex3.ItemIndex := ItemEffect.FileIndex3 + 1;

    EditItemEffectImageCount1.Value := ItemEffect.ImageCount1;
    EditItemEffectImageCount2.Value := ItemEffect.ImageCount2;
    EditEffectDesc.Text := ItemEffect.EffectDesc;
...
procedure TfrmFunctionConfig.ButtonEffectAddClick(Sender: TObject);
var
  ItemEffect: pTItemEffect;
  StdItem: pTStdItem;
begin
  if EditEffectIndex.Value <= 0 then begin
    Application.MessageBox('特效编号必须大于0！！！', '提示信息', MB_OK + MB_ICONWARNING);
    Exit;
  end;
  if GetItemEffect(EditEffectIndex.Value) <> nil then begin
    Application.MessageBox('此特效编号已经存在！！！', '提示信息', MB_OK + MB_ICONWARNING);
    Exit;
  end;
  New(ItemEffect);
  ...
  ItemEffect.Offset1 := EditItemEffectOffset1.Value;
  ItemEffect.Offset2 := EditItemEffectOffset2.Value;
  ItemEffect.Offset3 := EditItemEffectOffset3.Value;

  ItemEffect.FileIndex1 := _MAX(ComboBoxEffectFileIndex1.ItemIndex - 1, -1);
  ItemEffect.FileIndex2 := _MAX(ComboBoxEffectFileIndex2.ItemIndex - 1, -1);
  ItemEffect.FileIndex3 := _MAX(ComboBoxEffectFileIndex3.ItemIndex - 1, -1);

  ItemEffect.ImageCount1 := EditItemEffectImageCount1.Value;
  ItemEffect.ImageCount2 := EditItemEffectImageCount2.Value;

  g_EffectList.Add(ItemEffect);
  RefEffectList;
  ButtonEffectSave.Enabled := True;
  Application.MessageBox('增加成功！！！', '提示信息', MB_OK + MB_ICONWARNING);
end;

procedure TfrmFunctionConfig.ButtonEffectChgClick(Sender: TObject);
var
  I, nIndex: Integer;
  ItemEffect: pTItemEffect;
  StdItem: pTStdItem;
begin
  ...
  for I := 0 to g_EffectItemList.Count - 1 do begin
    ...
    if (StdItem <> nil) and (StdItem.Effect <> nil) and (StdItem.Effect = ItemEffect) then begin
      StdItem.BodyEffectFileIndex := ItemEffect.FileIndex1; //内观物品发光效果 文件编号 0
      StdItem.BodyEffectStartOffSet := ItemEffect.OffSet1; // 内观物品发光效果 读取位置
      StdItem.BodyEffectImageCount := ItemEffect.ImageCount1; // 内观物品发光效果 读取张数

      StdItem.EffectFileIndex := ItemEffect.FileIndex2; //包裹中的物品发光效果 文件编号 0
      StdItem.EffectStartOffSet := ItemEffect.OffSet2; // 包裹中的物品发光效果 读取位置
      StdItem.EffectImageCount := ItemEffect.ImageCount2; // 包裹中的物品发光效果 读取张数
    end;
  end;
...
procedure TfrmFunctionConfig.ButtonEffectDelClick(Sender: TObject);
var
  I, nIndex: Integer;
  ItemEffect: pTItemEffect;
  StdItem: pTStdItem;
begin
  ...
  if nIndex >= 0 then begin
    ...
    for I := 0 to g_EffectItemList.Count - 1 do begin
      StdItem := UserEngine.GetStdItem(g_EffectItemList.Strings[I]);
      if (StdItem <> nil) and (StdItem.Effect <> nil) and (StdItem.Effect = ItemEffect) then begin
        StdItem.EffectFileIndex := -1; //包裹中的物品发光效果 文件编号 0
        StdItem.EffectStartOffSet := 0; // 包裹中的物品发光效果 读取位置
        StdItem.EffectImageCount := 0; // 包裹中的物品发光效果 读取张数

        StdItem.BodyEffectFileIndex := -1; //内观物品发光效果 文件编号 0
        StdItem.BodyEffectStartOffSet := 0; // 内观物品发光效果 读取位置
        StdItem.BodyEffectImageCount := 0; // 内观物品发光效果 读取张数
        StdItem.Effect := nil;
      end;
    end;

    g_EffectList.Remove(ItemEffect);
    RefEffectList;
    ButtonEffectDel.Enabled := False;
    ButtonEffectSave.Enabled := True;
...
procedure TfrmFunctionConfig.ListBoxWilNameListClick(Sender: TObject);
begin
  if ListBoxWilNameList.ItemIndex >= 0 then begin
    ButtonWilNameDown.Enabled := (ListBoxWilNameList.ItemIndex >= 0) and (ListBoxWilNameList.ItemIndex < ListBoxWilNameList.Count - 1);
    ButtonWilNameUP.Enabled := (ListBoxWilNameList.ItemIndex > 0);
    ...
    ButtonWilDel.Enabled := True;
  end else ButtonWilDel.Enabled := False;
end;

procedure TfrmFunctionConfig.ButtonWilNameUPClick(Sender: TObject);
var
  sWilName: string;
  ItemIndex: Integer;
begin
...
procedure TfrmFunctionConfig.ButtonWilNameDownClick(Sender: TObject);
var
  sWilName: string;
  ItemIndex: Integer;
begin
  ItemIndex := ListBoxWilNameList.ItemIndex;
  if (ItemIndex >= 0) and (ItemIndex < ListBoxWilNameList.Count - 1) then begin
    sWilName := ListBoxWilNameList.Items[ItemIndex];
    ListBoxWilNameList.DeleteSelected;
    ListBoxWilNameList.Items.Insert(ItemIndex + 1, sWilName);
    ListBoxWilNameList.ItemIndex := ItemIndex + 1;
...
procedure TfrmFunctionConfig.ButtonWilAddClick(Sender: TObject);
var
  I: Integer;
  sWilName: string;
begin
  sWilName := Trim(EditWilName.Text);
  if sWilName = '' then begin
    Application.MessageBox('请输入WIL文件名称！！！', '错误信息', MB_OK + MB_ICONERROR);
    EditWilName.SetFocus;
    Exit;
  end;
  for I := 0 to ListBoxWilNameList.Items.Count - 1 do begin
    if CompareText(ListBoxWilNameList.Items.Strings[I], sWilName) = 0 then begin
...
procedure TfrmFunctionConfig.ButtonWilDelClick(Sender: TObject);
begin
  if ListBoxWilNameList.ItemIndex >= 0 then begin
    ListBoxWilNameList.DeleteSelected;
    ButtonWilDel.Enabled := False;
...
procedure TfrmFunctionConfig.ButtonWilSaveClick(Sender: TObject);
begin
  g_EffectImageList.Clear;
  g_EffectImageList.AddStrings(ListBoxWilNameList.Items);

  ComboBoxEffectFileIndex1.Clear;
  ComboBoxEffectFileIndex2.Clear;
  ComboBoxEffectFileIndex3.Clear;
...
procedure TfrmFunctionConfig.ListBoxStdItemListKeyDown(Sender: TObject;
  ...
  case Key of
    Word('F'): begin
        if ssCtrl in Shift then begin
          Key := 0;
          sItemName := '';
          if not InputQuery('物品查找', '输入物品名称:', sItemName) then Exit;
          if sItemName = '' then Exit;
          for I := 0 to TListBox(Sender).Items.Count - 1 do begin
            if TListBox(Sender).Items.Strings[I] = sItemName then begin
...
procedure TfrmFunctionConfig.ListBoxStdItemListClick(Sender: TObject);
begin
  if ListBoxStdItemList.ItemIndex >= 0 then
    EditEffectItemName.Text := ListBoxStdItemList.Items.Strings[ListBoxStdItemList.ItemIndex];
end;

procedure TfrmFunctionConfig.ListBoxEffectItemListClick(Sender: TObject);
var
  I, nIndex: Integer;
  ItemEffect: pTItemEffect;
begin
  ...
  if nIndex >= 0 then begin
    ...
    for I := 0 to ComboBoxEffectIndex.Items.Count - 1 do begin
      if pTItemEffect(ComboBoxEffectIndex.Items.Objects[I]) = ItemEffect then begin
...
procedure TfrmFunctionConfig.ComboBoxEffectIndexChange(Sender: TObject);
...
begin
  ...
  if nIndex >= 0 then begin
    ...
    S := ComboBoxEffectIndex.Items.Strings[nIndex] + ':' + #13#10#13#10;
    if ItemEffect.FileIndex1 < 0 then begin
      S := S + '内观特效: 无' + #13#10;
    end else begin
      S := S + Format('内观特效:' + #13#10 + '读取%s' + #13#10 + '开始图片:%d' + #13#10 + '播放图片张数:%d' + #13#10, [GetImageName(ItemEffect.FileIndex1), ItemEffect.OffSet1, ItemEffect.ImageCount1]) + #13#10;
    end;

    if ItemEffect.FileIndex3 < 0 then begin
      S := S + '外观特效: 无' + #13#10;
    end else begin
      S := S + Format('外观特效:' + #13#10 + '读取%s' + #13#10 + '开始图片:%d' + #13#10, [GetImageName(ItemEffect.FileIndex3), ItemEffect.OffSet3]) + #13#10;
    end;

    if ItemEffect.FileIndex2 < 0 then begin
      S := S + '包裹中的特效: 无';
    end else begin
      S := S + Format('包裹中的特效:' + #13#10 + '读取%s' + #13#10 + '开始图片:%d' + #13#10 + '播放图片张数:%d', [GetImageName(ItemEffect.FileIndex2), ItemEffect.OffSet2, ItemEffect.ImageCount2]);
    end;
    LabelEffectMemo.Caption := Trim(S);
    if ListBoxEffectItemList.ItemIndex >= 0 then begin
      ButtonEffectItemChg.Enabled := True;
      ButtonEffectItemDel.Enabled := True;
    end;
  end;
...
procedure TfrmFunctionConfig.ButtonEffectItemAddClick(Sender: TObject);
var
  I, nIndex: Integer;
  sItemName: string;
  ItemEffect: pTItemEffect;
...
begin
  ...
  if GetEffectItemList(sItemName) >= 0 then begin
...
begin
  ...
  if nIndex >= 0 then begin
    ...
    StdItem.Effect := ItemEffect;
    for I := 0 to ListBoxEffectItemList.Items.Count - 1 do begin
      if CompareText(ListBoxEffectItemList.Items.Strings[I], sItemName) = 0 then begin
    ...
    ButtonEffectItemSave.Enabled := True;
    Application.MessageBox('修改成功！！！', '提示信息', MB_OK + MB_ICONWARNING);
  end else Application.MessageBox('修改失败！！！', '提示信息', MB_OK + MB_ICONWARNING);
end;

procedure TfrmFunctionConfig.ButtonEffectItemDelClick(Sender: TObject);
var
  I, nIndex: Integer;
  sItemName: string;
  ItemEffect: pTItemEffect;
...
begin
  ...
  if StdItem <> nil then begin
    for I := 0 to ListBoxEffectItemList.Items.Count - 1 do begin
      if pTItemEffect(ListBoxEffectItemList.Items.Objects[I]).Index = Integer(g_EffectItemList.Objects[nIndex]) then begin
        StdItem.EffectFileIndex := -1; //包裹中的物品发光效果 文件编号 0
        StdItem.EffectStartOffSet := 0; // 包裹中的物品发光效果 读取位置
        StdItem.EffectImageCount := 0; // 包裹中的物品发光效果 读取张数
...
procedure TfrmFunctionConfig.ButtonEffectItemSaveClick(Sender: TObject);
begin
  ButtonEffectItemSave.Enabled := False;
  ButtonEffectItemChg.Enabled := False;
  ButtonEffectItemDel.Enabled := False;
...
procedure TfrmFunctionConfig.ButtonEffectSaveClick(Sender: TObject);
begin
  ButtonEffectSave.Enabled := False;
  ButtonEffectChg.Enabled := False;
  ButtonEffectDel.Enabled := False;
...
procedure TfrmFunctionConfig.ComboBoxEffectFileIndex2Change(
  Sender: TObject);
begin
  if ComboBoxEffectFileIndex2.ItemIndex = 0 then begin
    EditItemEffectOffset2.Value := 0;
    EditItemEffectImageCount2.Value := 0;
  end;
end;

procedure TfrmFunctionConfig.ComboBoxEffectFileIndex1Change(
  Sender: TObject);
begin
  if ComboBoxEffectFileIndex1.ItemIndex = 0 then begin
    EditItemEffectOffset1.Value := 0;
    EditItemEffectImageCount1.Value := 0;
  end;
end;

procedure TfrmFunctionConfig.ComboBoxEffectFileIndex3Change(
  Sender: TObject);
begin
  if ComboBoxEffectFileIndex3.ItemIndex = 0 then begin
    EditItemEffectOffset3.Value := 0;
  end;
end;
