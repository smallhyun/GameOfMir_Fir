...
type
  ...
  public
    ...
    function GetSellListByItemNameX(sItemName: string; var SellList: TSellList): Integer; //模糊搜索
...
implementation
uses
  M2Share, RunDB, ObjBase, HUtil32, StrUtils;
      ...
      if FileRead(m_nFileHandle, DBHeader, SizeOf(TItemCount)) = SizeOf(TItemCount) then begin
        for nIndex := 0 to DBHeader - 1 do begin
          ...
          if (DBRecord.UserItem.wIndex > 0) and (DBRecord.sCharName <> '') then begin
            ...
            if DBRecord.UserItem.btValue2[0] = 0 then begin
              DBRecord.UserItem.btValue2[0] := 1;
              DBRecord.UserItem.btColor := 251;
...
function TSellEngine.GetSellListByItemNameX(sItemName: string; var SellList: TSellList): Integer;
var
  I: Integer;
begin
  Lock();
  try
    ...
    if sItemName <> '' then begin
      for I := 0 to m_ItemSellList.Count - 1 do begin
        if AnsiContainsText(m_ItemSellList.Strings[I], sItemName) then begin
          SellList := TSellList(m_ItemSellList.Objects[I]);
          Result := I;
          break;
        end;
      end;
