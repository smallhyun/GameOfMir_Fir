...
interface
uses
  Windows, Classes, SysUtils, Grobal2, D7ScktComp, WinSock, Common, M2Share;
function DBSocketWorking(): Boolean;
function DBSocketConnected(): Boolean;
function GetDBSockMsg(nQueryID: Integer; var DefMsg: TDefaultMessage; var sDefMsg, sStr: string; dwTimeOut: LongWord; SockData: TSockData): Boolean;
...
begin
  ...
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sStr, g_Config.dwGetDBSockMsgTime {5000}, d_SaveData) then begin
    if (DefMsg.Ident = DBR_SAVEMAGICLIST) and (DefMsg.Recog = 1) then
      Result := True;
  end;
end;
...
begin
  ...
  SendDBSockMsg(nQueryID, sSendText);
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sStr, g_Config.dwGetDBSockMsgTime {5000}, d_SaveData) then begin
    if (DefMsg.Ident = DBR_SAVESTDITEMLIST) and (DefMsg.Recog = 1) then
      Result := True;
  end;
end;
...
begin
  ...
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sStr, g_Config.dwGetDBSockMsgTime, d_LoadHeroData) then begin
    //MainOutMessage('DeleteHeroRcd nRecog:'+IntToStr(nRecog));
    nStatus := DefMsg.Recog;
    if (DefMsg.Ident = DBR_DELHERORCD) and (DefMsg.Recog = 1) then begin
      Result := True;
    end;
  end;
  Inc(g_Config.nSaveDBCount);
...
begin
  ...
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sHumanRcdStr, g_Config.dwGetDBSockMsgTime {5000}, d_LoadHeroData) then begin
    //MainOutMessage('CreateHeroRcd nRecog:'+IntToStr(nRecog));
    Result := False;
    nStatus := DefMsg.Recog;
    if DefMsg.Ident = DBR_NEWHERORCD then begin
...
begin
  ...
  Result := False;
  n4EBB68 := 101;
  sStr := zEncodeBuffer(@HumanRcd, SizeOf(THumDataInfo));
  SendDBSockMsg(nQueryID, EncodeMessage(MakeDefaultMsg(DB_SAVEHERORCD, nSessionID, 0, 0, Length(sStr))) + EncodeString(sAccount) + '/' + EncodeString(sCharName) + '/' + sStr);
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sStr, g_Config.dwGetDBSockMsgTime {5000}, d_SaveHeroData) then begin
    if (DefMsg.Ident = DBR_SAVEHERORCD) and (DefMsg.Recog = 1) then
      Result := True;
  end;
end;
...
begin
  ...
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sHumanRcdStr, g_Config.dwGetDBSockMsgTime {5000}, d_LoadHeroData) then begin
    Result := False;
    if DefMsg.Ident = DBR_LOADHERORCD then begin
      if DefMsg.Recog = 1 then begin
        ...
        if sDBCharName = sCharName then begin
          ...
          if DefMsg.Series = Length(sHumanRcdStr) then begin
            zDecodeBuffer(sHumanRcdStr, @HumanRcd, SizeOf(THumDataInfo));
            Result := HumanRcd.Data.boIsHero;
          end;
        end else Result := False;
      end else Result := False;
    end else
      if DefMsg.Ident = DBR_NOTCREATEHERO then begin //没有创建英雄
...
begin
  ...
  n4EBB68 := 101;
  sStr := zEncodeBuffer(@HumanRcd, SizeOf(THumDataInfo));
  SendDBSockMsg(nQueryID, EncodeMessage(MakeDefaultMsg(DB_SAVEHUMANRCD, nSessionID, 0, 0, Length(sStr))) + EncodeString(sAccount) + '/' + EncodeString(sCharName) + '/' + sStr);
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sStr, g_Config.dwGetDBSockMsgTime {5000}, d_SaveHumData) then begin
    if (DefMsg.Ident = DBR_SAVEHUMANRCD) and (DefMsg.Recog = 1) then
      Result := True;
  end;
end;
...
begin
  ...
  SendDBSockMsg(nQueryID, sDBMsg);
  //MainOutMessage('LoadRcd');
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sHumanRcdStr, g_Config.dwGetDBSockMsgTime {5000}, d_LoadHumData) then begin
    Result := False;
    //MainOutMessage('LoadRcd 0');
    if DefMsg.Ident = DBR_LOADHUMANRCD then begin
      if DefMsg.Recog = 1 then begin
        //MainOutMessage('LoadRcd 1');
        sHumanRcdStr := GetValidStr3(sHumanRcdStr, sDBMsg, ['/']);
        sDBCharName := DeCodeString(sDBMsg);
        if sDBCharName = sCharName then begin
           //MainOutMessage('LoadRcd 2');
          //if GetCodeMsgSize(SizeOf(THumDataInfo) * 4 / 3) = Length(sHumanRcdStr) then begin
          if DefMsg.Series = Length(sHumanRcdStr) then begin
            //MainOutMessage('LoadRcd 3');
            zDecodeBuffer(sHumanRcdStr, @HumanRcd, SizeOf(THumDataInfo));
...
begin
  ...
  if GetDBSockMsg(nQueryID, DefMsg, sDefMsg, sRankingStr, g_Config.dwGetDBSockMsgTime {5000}, d_LoadRankingData) then begin
    Result := False;
    if DefMsg.Ident = DBR_LOADRANKING then begin
      DefMsg := DecodeMessage(sDefMsg);
      Msg^ := DefMsg;
      Result := True;
