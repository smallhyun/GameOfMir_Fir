...
function GetActorObject(NPC: TNormNpc; PlayObject: TPlayObject): TActorObject;   }
  ...
  private
    ...
    procedure ActionOfChangeItemNewAddValue(BaseObject: TActorObject; QuestActionInfo: pTQuestActionInfo);
    ...
    procedure ActionOfUnBindUseItem(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfStartMoveItemValue(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfStopMoveItemValue(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfMoveItemValue(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfSetMoveItemValue(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfMoveItemValueFinish(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfSetActiveAttackHum(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfSetCheckUsePlugIn(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfChangeModeEx(BaseObject: TActorObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfChangeSpeed(BaseObject: TActorObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfPlayEffect(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfScreenEffect(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);

    procedure ActionOfRecallSelf(BaseObject: TActorObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfCopyHumUseItems(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfRangeGoto(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);


    procedure ActionOfCreateGroupFail(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
    procedure ActionOfChangeItemColor(BaseObject: TActorObject; QuestActionInfo: pTQuestActionInfo);
    ...
    function ConditionOfCheckMapAIHumanCount(BaseObject: TActorObject; QuestConditionInfo: pTQuestConditionInfo): Boolean;
    ...
    function ConditionOfCheckItemDura(BaseObject: TActorObject; QuestConditionInfo: pTQuestConditionInfo): Boolean;
    function ConditionOfCheckItemNewAddValueCount(BaseObject: TActorObject; QuestConditionInfo: pTQuestConditionInfo): Boolean;
    ...
    function ConditionOfCheckItemColor(BaseObject: TActorObject; QuestConditionInfo: pTQuestConditionInfo): Boolean;
  public
...
begin
  ...
  if Assigned(PlugInEngine.CheckCanUpgradeItem) then begin //禁止升级
    ...
    if PlugInEngine.CheckCanUpgradeItem(User, PAnsiChar(sCheckItemName), True) then begin
      Exit;
    end;
  end;
    ...
    if (User.m_sHeroCharName <> '') then begin
      ...
      if (Length(sMsg) > 0) and (Length(sMsg) < 15) then begin
        if Assigned(PlugInEngine.PlayObjectFilterMsg) then begin
          sSrcMsg := sMsg;
          if PlugInEngine.PlayObjectFilterMsg(User, PAnsiChar(sSrcMsg), @DestMsg, boGotoLabel) then begin
      ...
      if {not PlayObject.m_boDeath and}(sData <> '') and (sData[1] = '@') then begin //修改死亡后可以点击NPC
        ...
        if CompareText(sLabel, sSL_SENDMSG) = 0 then begin
          ...
          if Assigned(PlugInEngine.PlayObjectUserSelect) then begin
            PlugInEngine.PlayObjectUserSelect(Self, PlayObject, PAnsiChar(sLabel), PAnsiChar(sMsg));
          end;
        end;
      end;
    end;
  ...
  if n10 > 0 then begin
    ...
    if (StdItem <> nil) and
      ...
      if (StdItem.StdMode = 43) then begin
        //if UserItem.DuraMax < 10000 then UserItem.DuraMax := 10000; 修复矿石纯度越高出售价格越低的问题
        if UserItem.Dura <= UserItem.DuraMax then begin
          n20 := (n10 / 2.0 / UserItem.DuraMax * (UserItem.DuraMax - UserItem.Dura));
          n10 := _MAX(2, Round(n10 - n20));
        end else begin
...
begin
  if Assigned(PlugInEngine.MerchantClientGetDetailGoodsList) then begin
    PlugInEngine.MerchantClientGetDetailGoodsList(Self, PlayObject, PAnsiChar(sItemName), nInt);
  end else begin
...
begin
  ...
  if Assigned(PlugInEngine.CheckCanSellItem) then begin //禁止出售
    ...
    if PlugInEngine.CheckCanSellItem(PlayObject, PAnsiChar(sCheckItemName), True) then begin
...
var
  ...
  Guild: TGuild;
begin
  //MainOutMessage('ActionOfAddGuildMember1');
  if PlayObject.m_MyGuild <> nil then begin
    PlayObject.SysMsg('你已经有行会了！！！', c_Green, t_Hint);
    Exit;
  end;
  sGuildName := QuestActionInfo.sParam1;
  GetVarValue(PlayObject, QuestActionInfo.sParam1, sGuildName);
  //sGuildName := GetLineVariableText(PlayObject, sGuildName);
  //MainOutMessage('ActionOfAddGuildMember2:'+sGuildName);
  if sGuildName = '' then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_ADDGUILDMEMBER);
    Exit;
  end;
  //MainOutMessage('ActionOfAddGuildMember3:'+sGuildName);
  Guild := g_GuildManager.FindGuild(sGuildName);
  if Guild = nil then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_ADDGUILDMEMBER);
    Exit;
  end;
  //MainOutMessage('ActionOfAddGuildMember4:'+sGuildName);
  if not Guild.IsMember(PlayObject.m_sCharName) then begin
     //MainOutMessage('ActionOfAddGuildMember5:'+sGuildName);
    if not Guild.IsFull then begin
      Guild.AddMember(PlayObject);
      PlayObject.m_MyGuild := Guild;
      PlayObject.m_sGuildRankName := Guild.GetRankName(PlayObject, PlayObject.m_nGuildRankNo);
      //PlayObject.RefRankInfo(PlayObject.m_nGuildRankNo, PlayObject.m_sGuildRankName);
      PlayObject.SendMsg(PlayObject, RM_CHANGEGUILDNAME, 0, 0, 0, 0, '');
      PlayObject.RefShowName();
      PlayObject.SysMsg('你已加入行会: ' + TGUild(PlayObject.m_MyGuild).sGuildName + ' 当前封号为: ' + PlayObject.m_sGuildRankName, c_Green, t_Hint);
      //MainOutMessage('ActionOfAddGuildMember6:'+sGuildName);
    end else begin
      PlayObject.SysMsg(sGuildName + ' 行会成员数最高限制为：' + IntToStr(TGUild(PlayObject.m_MyGuild).m_nMemberMaxLimit), c_Green, t_Hint);
      //MainOutMessage('ActionOfAddGuildMember7:'+sGuildName);
    end;
  end else begin
    PlayObject.SysMsg('你已经在行会中了！！！', c_Green, t_Hint);
    //MainOutMessage('ActionOfAddGuildMember8:'+sGuildName);
  end;
end;
...
begin
  ...
  if Assigned(PlugInEngine.CheckCanUpgradeItem) then begin //禁止升级
    ...
    if PlugInEngine.CheckCanUpgradeItem(PlayObject, PAnsiChar(sCheckItemName), True) then begin
      Exit;
    end;
  end;
  case cMethod of
...
begin
  ...
  if (PlayObject.m_nItemIndex >= 0) and (PlayObject.m_nItemIndex < PlayObject.m_ItemList.Count) then begin
    ...
    if (UserItem.MakeIndex <> PlayObject.m_UpgradeUserItem.MakeIndex) then begin
...
begin
  ...
  if (PlayObject.m_nItemIndex >= 0) and (PlayObject.m_nItemIndex < PlayObject.m_ItemList.Count) then begin
    UserItem := pTUserItem(PlayObject.m_ItemList.Items[PlayObject.m_nItemIndex]);
    if (UserItem.MakeIndex <> PlayObject.m_UpgradeUserItem.MakeIndex) or (not IsRememberItem(UserItem)) then begin
    ...
    if FileExists(sFileName) then begin
      ...
      if ItemIni <> nil then begin
        ...
        if PlayObject <> nil then begin
          PlayObject.m_boActiveAttackHum := ItemIni.ReadBool('BaseInfo', 'Attack', PlayObject.m_boActiveAttackHum);
          PlayObject.m_boAIAutoSay := ItemIni.ReadBool('BaseInfo', 'Say', PlayObject.m_boAIAutoSay);
          PlayObject.m_boAIAutoPickUpItem := ItemIni.ReadBool('BaseInfo', 'PickUpItem', PlayObject.m_boAIAutoPickUpItem);
        end;

        BaseObject.m_nRobotRandomAddValue := ItemIni.ReadInteger('BaseInfo', 'RobotRandomAddValue', g_Config.nRobotRandomAddValue);
        BaseObject.m_nRobotRandomNotLimit := ItemIni.ReadInteger('BaseInfo', 'RobotRandomNotLimit', g_Config.nRobotRandomNotLimit);
        BaseObject.m_nRobotRandomNewAddValue := ItemIni.ReadInteger('BaseInfo', 'RobotRandomNewAddValue', g_Config.nRobotRandomNewAddValue);
        BaseObject.m_nRobotRandomAddPoint := ItemIni.ReadInteger('BaseInfo', 'RobotRandomAddPoint', g_Config.nRobotRandomAddPoint);
        TAIObject(BaseObject).m_nDieDropUseItemRate := ItemIni.ReadInteger('BaseInfo', 'DieDropUseItemRate', 100);
        BaseObject.m_boButchItemEx := ItemIni.ReadBool('BaseInfo', 'ButchItem', False);
        BaseObject.m_boButchItem := BaseObject.m_boButchItemEx;
        BaseObject.m_boNoDropItem := ItemIni.ReadBool('BaseInfo', 'NoDropItem', False);
        BaseObject.m_boNoDropUseItemConfig := ItemIni.ReadBool('BaseInfo', 'NoDropUseItem', False);
        BaseObject.m_boDropBagItemConfig := ItemIni.ReadBool('BaseInfo', 'DropBagItem', BaseObject.m_boDropBagItemConfig);
          ...
          for I := 0 to TempList.Count - 1 do begin
            ...
            if StdItem <> nil then begin
              ...
              if UserEngine.CopyToUserItemFromName(sItemName, UserItem) then begin
                ...
                if Random(BaseObject.m_nRobotRandomNewAddValue {10}) = 0 then
                  UserEngine._RandomUpgradeItem(UserItem);

                if Random(BaseObject.m_nRobotRandomAddPoint) = 0 then //新增5属性
                  UserEngine.RandomItemAddPoint(UserItem);

                UserEngine._RandomItemLimitDay(UserItem, BaseObject.m_nRobotRandomNotLimit);

                if Random(BaseObject.m_nRobotRandomAddValue {10}) = 0 then
        ...
        if StdItem <> nil then begin
          if (BaseObject.m_UseItems[nWhere].DuraMax > BaseObject.m_UseItems[nWhere].Dura) and (StdItem.StdMode <> 43) then begin
            if Assigned(PlugInEngine.CheckCanRepairItem) then begin
              sCheckItemName := StdItem.Name;
              if not PlugInEngine.CheckCanRepairItem(BaseObject, PAnsiChar(sCheckItemName), False) then Exit;
            end;
      ...
      if BaseObject.m_UseItems[nWhere].wIndex > 0 then begin
        ...
        if StdItem <> nil then begin
          if (BaseObject.m_UseItems[nWhere].DuraMax > BaseObject.m_UseItems[nWhere].Dura) and (StdItem.StdMode <> 43) then begin
            if Assigned(PlugInEngine.CheckCanRepairItem) then begin
              sCheckItemName := StdItem.Name;
              if not PlugInEngine.CheckCanRepairItem(BaseObject, PAnsiChar(sCheckItemName), False) then Continue;
            end;
            BaseObject.m_UseItems[nWhere].Dura := BaseObject.m_UseItems[nWhere].DuraMax;
            BaseObject.SendMsg(BaseObject, RM_DURACHANGE, nWhere, BaseObject.m_UseItems[nWhere].Dura, BaseObject.m_UseItems[nWhere].DuraMax, 0, '');
          end;
...
begin
  ...
  if CompareLStr(sVariable, '$USEITEMS[', Length('$USEITEMS[')) then begin //身上装备 m_UseItems
    n18 := Pos(']', sVariable);
    if n18 > 0 then begin
      s14 := Copy(sVariable, Length('$USEITEMS[') + 1, n18 - Length('$USEITEMS[') - 1);
      n18 := StrToIntDef(s14, -1);
      //MainOutMessage('s14:' + s14);
      if n18 in [Low(THumanUseItems)..High(THumanUseItems)] then begin
        n20 := Pos('.', sVariable);
        if n20 > 0 then begin
          if PlayObject.m_UseItems[n18].wIndex <= 0 then begin
            sMsg := sub_49ADB8(sMsg, sVariable, '');
            Exit;
          end;
          s14 := Copy(sVariable, n20 + 1, Length(sVariable) - n20);

          if s14 = 'INDEX' then begin
            sText := IntToStr(PlayObject.m_UseItems[n18].wIndex);
            sMsg := sub_49ADB8(sMsg, Format('<$USEITEMS[%d].INDEX>', [n18]), sText);
            Exit;
          end;
          if s14 = 'MAKEINDEX' then begin
            sText := IntToStr(PlayObject.m_UseItems[n18].MakeIndex);
            sMsg := sub_49ADB8(sMsg, Format('<$USEITEMS[%d].MAKEINDEX>', [n18]), sText);
            Exit;
          end;
          if s14 = 'DURA' then begin
            sText := IntToStr(PlayObject.m_UseItems[n18].Dura);
            sMsg := sub_49ADB8(sMsg, Format('<$USEITEMS[%d].DURA>', [n18]), sText);
            Exit;
          end;
          if s14 = 'DURAMAX' then begin
            sText := IntToStr(PlayObject.m_UseItems[n18].DuraMax);
            sMsg := sub_49ADB8(sMsg, Format('<$USEITEMS[%d].DURAMAX>', [n18]), sText);
            Exit;
          end;
          if s14 = 'NAME' then begin
            sText := UserEngine.GetStdItemName(PlayObject.m_UseItems[n18].wIndex);
            sMsg := sub_49ADB8(sMsg, Format('<$USEITEMS[%d].NAME>', [n18]), sText);
            Exit;
          end;
        end;
      end;
  ...
  if CompareLStr(sVariable, '$HERO.USEITEMS[', Length('$HERO.USEITEMS[')) then begin //身上装备 m_UseItems
    n18 := Pos(']', sVariable);
    if n18 > 0 then begin
      if PlayObject.m_MyHero = nil then begin
        sMsg := sub_49ADB8(sMsg, sVariable, '');
      ...
      s14 := Copy(sVariable, Length('$HERO.USEITEMS[') + 1, n18 - Length('$HERO.USEITEMS[') - 1);
      n18 := StrToIntDef(s14, -1);
      if n18 in [Low(THumanUseItems)..High(THumanUseItems)] then begin
        n20 := Pos('.', sVariable);
        s1C := Copy(sVariable, n20 + 1, Length(sVariable) - n20);
        n20 := Pos('.', s1C);
        if n20 > 0 then begin
          s14 := Copy(s1C, n20 + 1, Length(s1C) - n20);
          if s14 = 'INDEX' then begin
            sText := IntToStr(PlayObject.m_MyHero.m_UseItems[n18].wIndex);
            sMsg := sub_49ADB8(sMsg, Format('<$HERO.USEITEMS[%d].INDEX>', [n18]), sText);
            Exit;
          end;
          if s14 = 'MAKEINDEX' then begin
            sText := IntToStr(PlayObject.m_MyHero.m_UseItems[n18].MakeIndex);
            sMsg := sub_49ADB8(sMsg, Format('<$HERO.USEITEMS[%d].MAKEINDEX>', [n18]), sText);
            Exit;
          end;
          if s14 = 'DURA' then begin
            sText := IntToStr(PlayObject.m_MyHero.m_UseItems[n18].Dura);
            sMsg := sub_49ADB8(sMsg, Format('<$HERO.USEITEMS[%d].DURA>', [n18]), sText);
            Exit;
          end;
          if s14 = 'DURAMAX' then begin
            sText := IntToStr(PlayObject.m_MyHero.m_UseItems[n18].DuraMax);
            sMsg := sub_49ADB8(sMsg, Format('<$HERO.USEITEMS[%d].DURAMAX>', [n18]), sText);
            Exit;
          end;
          if s14 = 'NAME' then begin
            sText := UserEngine.GetStdItemName(PlayObject.m_MyHero.m_UseItems[n18].wIndex);
            sMsg := sub_49ADB8(sMsg, Format('<$HERO.USEITEMS[%d].NAME>', [n18]), sText);
            Exit;
          end;
        end;
      end;
  ...
  if sVariable = '$OK.INDEX' then begin
    sText := IntToSTR(PlayObject.m_UpgradeUserItem.wIndex);
    sMsg := sub_49ADB8(sMsg, '<$OK.INDEX>', sText);
    Exit;
  end;
  if sVariable = '$OK.MAKEINDEX' then begin
    sText := IntToSTR(PlayObject.m_UpgradeUserItem.MakeIndex);
    sMsg := sub_49ADB8(sMsg, '<$OK.MAKEINDEX>', sText);
    Exit;
  end;
  if sVariable = '$OK.DURA' then begin
    sText := IntToSTR(PlayObject.m_UpgradeUserItem.Dura);
    sMsg := sub_49ADB8(sMsg, '<$OK.DURA>', sText);
    Exit;
  end;
  if sVariable = '$OK.DURAMAX' then begin
    sText := IntToSTR(PlayObject.m_UpgradeUserItem.DuraMax);
    sMsg := sub_49ADB8(sMsg, '<$OK.DURAMAX>', sText);
    Exit;
  end;
  if sVariable = '$OK.NAME' then begin
    sText := PlayObject.m_UpgradeStdItem.Name;
    sMsg := sub_49ADB8(sMsg, '<$OK.NAME>', sText);
    Exit;
  end;
...
var
    ...
    for I := 0 to ConditionList.Count - 1 do begin
      ...
      case QuestConditionInfo.nCMDCode of
        nCHECK: begin
            ...
            if BaseObject.m_btRaceServer = RC_PLAYOBJECT then
              n10 := TPlayObject(BaseObject).GetQuestFalgStatus(n14)
            else
              if BaseObject.m_btRaceServer = RC_HEROOBJECT then
              n10 := THeroObject(BaseObject).GetQuestFalgStatus(n14);
        ...
        nSC_CHECKITEMNEWADDVALUECOUNT: if (not ConditionOfCheckItemNewAddValueCount(BaseObject, QuestConditionInfo)) then Result := False;
        ...
        nSC_CHECKMAPAIHUMANCOUNT: if (not IsPlayObject) or (not ConditionOfCheckMapAIHumanCount(PlayObject, QuestConditionInfo)) then Result := False;
        nSC_CHECKITEMDURA: if not ConditionOfCheckItemDura(BaseObject, QuestConditionInfo) then Result := False;
        nSC_CHECKITEMCOLOR: if not ConditionOfCheckItemColor(BaseObject, QuestConditionInfo) then Result := False;
      else begin
          if Assigned(PlugInEngine.ConditionScriptProcess) then begin
            try
              if (not IsPlayObject) or (not PlugInEngine.ConditionScriptProcess(Self,
                ...
                PAnsiChar(QuestConditionInfo.sParam1),
                QuestConditionInfo.nParam1,
                PAnsiChar(QuestConditionInfo.sParam2),
                QuestConditionInfo.nParam2,
                PAnsiChar(QuestConditionInfo.sParam3),
                QuestConditionInfo.nParam3,
                PAnsiChar(QuestConditionInfo.sParam4),
                QuestConditionInfo.nParam4,
                PAnsiChar(QuestConditionInfo.sParam5),
                QuestConditionInfo.nParam5,
                PAnsiChar(QuestConditionInfo.sParam6),
                QuestConditionInfo.nParam6,
                PAnsiChar(QuestConditionInfo.sParam7),
                QuestConditionInfo.nParam7,
                PAnsiChar(QuestConditionInfo.sParam8),
                QuestConditionInfo.nParam8,
                PAnsiChar(QuestConditionInfo.sParam9),
                QuestConditionInfo.nParam9,
                PAnsiChar(QuestConditionInfo.sParam10),
  ...
  procedure TakeItem(sItemName: string; nItemCount: Integer; sItemCount: string);
  var
    I: Integer;
    UserItem: pTUserItem;
    StdItem: pTStdItem;
  ...
  begin
    ...
    if IsPlayObject then begin
      ...
      //if nCount < 0 then
      GetVarValue(PlayObject, sItemCount, nCount); //增加变量支持
    end;
    ...
    for I := 0 to ActionList.Count - 1 do begin
      ...
      case QuestActionInfo.nCMDCode of
        nSET: begin
            ...
            if BaseObject.m_btRaceServer = RC_PLAYOBJECT then
              TPlayObject(BaseObject).SetQuestFlagStatus(n28, n2C)
            else
              if BaseObject.m_btRaceServer = RC_HEROOBJECT then
              THeroObject(BaseObject).SetQuestFlagStatus(n28, n2C);
          end;
        nTAKE: begin
        ...
        nSC_CHANGEITEMNEWADDVALUE: ActionOfChangeItemNewAddValue(BaseObject, QuestActionInfo);
        ...
        nSC_ADDGUILDMEMBER: if IsPlayObject then ActionOfAddGuildMember(PlayObject, QuestActionInfo);
        nSC_UNBINDUSEITEM: if IsPlayObject then ActionOfUnBindUseItem(PlayObject, QuestActionInfo);
        nSC_STARTMOVEITEMVALUE: if IsPlayObject then ActionOfStartMoveItemValue(PlayObject, QuestActionInfo);
        nSC_STOPMOVEITEMVALUE: if IsPlayObject then ActionOfStopMoveItemValue(PlayObject, QuestActionInfo);
        nSC_MOVEITEMVALUE: if IsPlayObject then ActionOfMoveItemValue(PlayObject, QuestActionInfo);
        nSC_SETMOVEITEMVALUE: if IsPlayObject then ActionOfSetMoveItemValue(PlayObject, QuestActionInfo);
        nSC_MOVEITEMVALUEFINISH: if IsPlayObject then ActionOfMoveItemValueFinish(PlayObject, QuestActionInfo);

        nSC_SETACTIVEATTACKHUM: if IsPlayObject then ActionOfSetActiveAttackHum(PlayObject, QuestActionInfo);
        nSC_SETCHECKUSEPLUGIN: if IsPlayObject then ActionOfSetCheckUsePlugIn(PlayObject, QuestActionInfo);
        nSC_CHANGEMODEEX: ActionOfChangeModeEx(BaseObject, QuestActionInfo);
        nSC_CHANGESPEED: if IsPlayObject then ActionOfChangeSpeed(PlayObject, QuestActionInfo);
        nSC_PLAYEFFECT: if IsPlayObject then ActionOfPlayEffect(PlayObject, QuestActionInfo);
        nSC_SCREENEFFECT: if IsPlayObject then ActionOfScreenEffect(PlayObject, QuestActionInfo);
        nSC_RECALLSELF: ActionOfRecallSelf(BaseObject, QuestActionInfo);
        nSC_COPYHUMUSEITEMS: if IsPlayObject then ActionOfCopyHumUseItems(PlayObject, QuestActionInfo);
        nSC_RANGEGOTO: if IsPlayObject then ActionOfRangeGoto(PlayObject, QuestActionInfo);
        nSC_CREATEGROUPFAIL: if IsPlayObject then ActionOfCreateGroupFail(PlayObject, QuestActionInfo);
        nSC_CHANGEITEMCOLOR: ActionOfChangeItemColor(BaseObject, QuestActionInfo);
      else begin
          if IsPlayObject and Assigned(PlugInEngine.ActionScriptProcess) then begin
            PlugInEngine.ActionScriptProcess(Self,
              ...
              PAnsiChar(QuestActionInfo.sParam1),
              QuestActionInfo.nParam1,
              PAnsiChar(QuestActionInfo.sParam2),
              QuestActionInfo.nParam2,
              PAnsiChar(QuestActionInfo.sParam3),
              QuestActionInfo.nParam3,
              PAnsiChar(QuestActionInfo.sParam4),
              QuestActionInfo.nParam4,
              PAnsiChar(QuestActionInfo.sParam5),
              QuestActionInfo.nParam5,
              PAnsiChar(QuestActionInfo.sParam6),
              QuestActionInfo.nParam6,
              PAnsiChar(QuestActionInfo.sParam7),
              QuestActionInfo.nParam7,
              PAnsiChar(QuestActionInfo.sParam8),
              QuestActionInfo.nParam8,
              PAnsiChar(QuestActionInfo.sParam9),
              QuestActionInfo.nParam9,
              PAnsiChar(QuestActionInfo.sParam10),
...
var
  sShowName: string;
begin
  if (m_nVarIdx >= 0) and (m_nVarIdx <= High(g_Config.GlobalAVal)) then begin
...
begin
  ...
  if (m_nVarIdx >= 0) and (m_nVarIdx <= High(g_Config.GlobalAVal)) then begin //动态更改NPC名称
    if CompareText(m_sDynamicName, g_Config.GlobalAVal[m_nVarIdx]) <> 0 then begin
      m_sDynamicName := g_Config.GlobalAVal[m_nVarIdx];
        ...
        if IsEnoughBag then begin
          ...
          if UserEngine.CopyToUserItemFromName(sItemName, UserItem) then begin
            ...
            if (BaseObject.m_btRaceServer = RC_PLAYOBJECT) and (QuestActionInfo.sParam3 = '1') then begin
              TPlayObject(BaseObject).m_UpgradeUserItem := UserItem^;
              TPlayObject(BaseObject).m_UpgradeStdItem := StdItem^;
...
begin
  ...
  if (nPosition < 1) then
    GetVarValue(PlayObject, QuestActionInfo.sParam1, nPosition);

  if (nCount < 1) then
    GetVarValue(PlayObject, QuestActionInfo.sParam3, nCount);

  if (nTime < 1) then
    GetVarValue(PlayObject, QuestActionInfo.sParam4, nTime);
  ...
  boAdd := QuestActionInfo.sParam5 = '1';

  case cMethod of
    '+': begin
        if not boAdd then begin
          PlayObject.m_wNewStatusArrValue[nPosition - 1] := 0;
          PlayObject.m_dwNewStatusArrTimeOutTick[nPosition - 1] := 0;
        end;
    ...
    '-': begin
        if not boAdd then begin
          PlayObject.m_wNewStatusArrValue[nPosition - 1 + 7] := 0;
          PlayObject.m_dwNewStatusArrTimeOutTick[nPosition - 1 + 7] := 0;
        end;
...
function TNormNpc.ConditionOfCheckItemColor(BaseObject: TActorObject; QuestConditionInfo: pTQuestConditionInfo): Boolean;
var
  cMethod: Char;
  nColor: Integer;
  nWhere: Integer;
...
begin
  ...
  nWhere := Str_ToInt(QuestConditionInfo.sParam3, -1);
  if (PlayObject <> nil) and (nWhere < 0) then
    GetVarValue(PlayObject, QuestConditionInfo.sParam3, nWhere); //增加变量支持

  if nWhere in [Low(THumanUseItems)..High(THumanUseItems)] then begin
    UserItem := @BaseObject.m_UseItems[nWhere];
    StdItem := UserEngine.GetStdItem(UserItem.wIndex);
    if (UserItem.wIndex <= 0) or (StdItem = nil) then begin
      BaseObject.SysMsg('你身上没有戴指定物品！！！', c_Red, t_Hint);
      Exit;
    end;
  end else begin
  ...
  if UserItem <> nil then begin
    cMethod := QuestConditionInfo.sParam1[1];
    nColor := Str_ToInt(QuestConditionInfo.sParam2, -1);
    if (PlayObject <> nil) and (nColor < 0) then
      GetVarValue(PlayObject, QuestConditionInfo.sParam2, nColor); //增加变量支持

    if (nColor >= 0) then begin
      case cMethod of
        '=': if UserItem.btColor = nColor then Result := True;
        '>': if UserItem.btColor > nColor then Result := True;
        '<': if UserItem.btColor < nColor then Result := True;
      else if UserItem.btColor >= nColor then Result := True;
      end;
    end else begin
      if (PlayObject <> nil) and (not (nWhere in [Low(THumanUseItems)..High(THumanUseItems)])) then
        PlayObject.SendChangeItemFail;
      ScriptConditionError(BaseObject, QuestConditionInfo, sSC_CHECKITEMCOLOR);
    end;
  end;
end;

function TNormNpc.ConditionOfCheckItemDura(BaseObject: TActorObject; QuestConditionInfo: pTQuestConditionInfo): Boolean;
var
  cMethod: Char;
  nDura: Integer;
  nWhere: Integer;
...
begin
  ...
  if UserItem <> nil then begin
    cMethod := QuestConditionInfo.sParam1[1];
    nDura := Str_ToInt(QuestConditionInfo.sParam2, -1);
    if (PlayObject <> nil) and (nDura < 0) then
      GetVarValue(PlayObject, QuestConditionInfo.sParam2, nDura); //增加变量支持

    if (nDura >= 0) then begin
      case cMethod of
        '=': if UserItem.Dura = nDura then Result := True;
        '>': if UserItem.Dura > nDura then Result := True;
        '<': if UserItem.Dura < nDura then Result := True;
      else if UserItem.Dura >= nDura then Result := True;
      end;
    end else begin
      if (PlayObject <> nil) and (not (nWhere in [Low(THumanUseItems)..High(THumanUseItems)])) then
        PlayObject.SendChangeItemFail;
      ScriptConditionError(BaseObject, QuestConditionInfo, sSC_CHECKITEMNEWADDVALUECOUNT);
    end;
  end;
end;

function TNormNpc.ConditionOfCheckItemNewAddValueCount(BaseObject: TActorObject;
  QuestConditionInfo: pTQuestConditionInfo): Boolean;
var
  cMethod: Char;
  nItemType: Integer;
...
begin
  ...
  nWhere := Str_ToInt(QuestConditionInfo.sParam4, -1);
  if (PlayObject <> nil) and (nWhere < 0) then
    GetVarValue(PlayObject, QuestConditionInfo.sParam4, nWhere); //增加变量支持
  ...
  if UserItem <> nil then begin
    ...
    nCount := Str_ToInt(QuestConditionInfo.sParam3, -1);
    cMethod := QuestConditionInfo.sParam2[1];
    if (nItemType in [1..17]) and (nCount >= 0) then begin
      if (nItemType in [1..5]) then begin
...
procedure TNormNpc.ActionOfStartMoveItemValue(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
begin
  if not PlayObject.m_boMoveItemValue then begin
    PlayObject.m_boMoveItemValue := True;
    PlayObject.SendDefMessage(SM_STARTMOVEITEMVALUE, 0, 0, 0, 0, '');
  end;
end;

procedure TNormNpc.ActionOfStopMoveItemValue(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
begin
  if PlayObject.m_boMoveItemValue then begin
    PlayObject.m_boMoveItemValue := False;
    PlayObject.m_SourceItem := nil; //源
    PlayObject.m_DestItem := nil; //目标

    FillChar(PlayObject.m_SourceLoBytes, SizeOf(PlayObject.m_SourceLoBytes), 0);
    FillChar(PlayObject.m_SourceHiBytes, SizeOf(PlayObject.m_SourceHiBytes), 0);

    FillChar(PlayObject.m_DestLoBytes, SizeOf(PlayObject.m_DestLoBytes), 0);
    FillChar(PlayObject.m_DestHiBytes, SizeOf(PlayObject.m_DestHiBytes), 0);


    PlayObject.m_SourceItemLight := nil; //源装备发光
    PlayObject.m_SourceStarCount := nil; //源星星数量

    PlayObject.m_DestItemLight := nil; //目标装备发光
    PlayObject.m_DestStarCount := nil; //目标星星数量

    FillChar(PlayObject.m_SourceAddPoint, SizeOf(PlayObject.m_SourceAddPoint), 0); //源
    FillChar(PlayObject.m_DestAddPoint, SizeOf(PlayObject.m_DestAddPoint), 0); //目标

    FillChar(PlayObject.m_SourceAyomTypes, SizeOf(PlayObject.m_SourceAyomTypes), 0); //源元素类型
    FillChar(PlayObject.m_SourceAyomValues, SizeOf(PlayObject.m_SourceAyomValues), 0); //源元素值

    FillChar(PlayObject.m_DestAyomTypes, SizeOf(PlayObject.m_DestAyomTypes), 0); //目标元素类型
    FillChar(PlayObject.m_DestAyomValues, SizeOf(PlayObject.m_DestAyomValues), 0); //目标元素值

    PlayObject.SendDefMessage(SM_STOPMOVEITEMVALUE, 0, 0, 0, 0, '');
  end;
end;

procedure TNormNpc.ActionOfSetActiveAttackHum(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
begin
  if PlayObject is TAIPlayObject then begin
    TAIPlayObject(PlayObject).m_boActiveAttackHum := QuestActionInfo.sParam1 = '1';
  end;
end;

procedure TNormNpc.ActionOfPlayEffect(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
var
  nFileIndex, nStartImage, nImageCount, nPlayCount, nTime: Integer;
  BaseObjectEffect: pTBaseObjectEffect;
begin
  nFileIndex := Str_ToInt(QuestActionInfo.sParam1, -1);
  nStartImage := Str_ToInt(QuestActionInfo.sParam2, -1);
  nImageCount := Str_ToInt(QuestActionInfo.sParam3, -1);
  nPlayCount := Str_ToInt(QuestActionInfo.sParam4, -1);
  nTime := Str_ToInt(QuestActionInfo.sParam5, -1);

  if (nFileIndex < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam1, nFileIndex);
    if (nFileIndex < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_PLAYEFFECT);
      Exit;
    end;
  end;

  if (nStartImage < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam2, nStartImage);
    if (nStartImage < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_PLAYEFFECT);
      Exit;
    end;
  ...
  if (nImageCount <= 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam3, nImageCount);
    if (nImageCount <= 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_PLAYEFFECT);
      Exit;
    end;
  ...
  if (nPlayCount < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam4, nPlayCount);
    if (nPlayCount <= 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_PLAYEFFECT);
      Exit;
    end;
  ...
  BaseObjectEffect.ActorEffect.wEffectImageOffSet := nStartImage;
  BaseObjectEffect.ActorEffect.btEffectImageCount := nImageCount;
  BaseObjectEffect.ActorEffect.wEffectFrameTime := nTime;
  BaseObjectEffect.ActorEffect.nLoopCount := nPlayCount;
  BaseObjectEffect.dwEffectTick := GetTickCount;
  PlayObject.m_BaseObjectEffects.Add(BaseObjectEffect);
  PlayObject.RefUseEffects;
end;

procedure TNormNpc.ActionOfScreenEffect(PlayObject: TPlayObject; QuestActionInfo: pTQuestActionInfo);
var
  I: Integer;
  nImageIndex, nStartImage, nImageCount, nPlayCount, nX, nY, nTime: Integer;
...
begin
  ...
  nImageIndex := Str_ToInt(QuestActionInfo.sParam3, -1);
  nStartImage := Str_ToInt(QuestActionInfo.sParam4, -1);
  nImageCount := Str_ToInt(QuestActionInfo.sParam5, -1);
  nPlayCount := Str_ToInt(QuestActionInfo.sParam6, -1);
  nTime := Str_ToInt(QuestActionInfo.sParam7, -1);

  if (nX < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam1, nX);
    if (nX < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_SCREENEFFECT);
      Exit;
    end;
  end;

  if (nY < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam2, nY);
    if (nY < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_SCREENEFFECT);
      Exit;
    end;
  ...
  if (nImageIndex < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam3, nImageIndex);
    if (nImageIndex < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_SCREENEFFECT);
      Exit;
    end;
  ...
  if (nStartImage < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam4, nStartImage);
    if (nStartImage < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_SCREENEFFECT);
      Exit;
    end;
  ...
  if (nImageCount < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam5, nImageCount);
    if (nImageCount < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_SCREENEFFECT);
      Exit;
    end;
  ...
  if (nPlayCount < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam6, nPlayCount);
    if (nPlayCount < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_SCREENEFFECT);
      Exit;
    end;
  ...
  if (nTime < 0) then begin
    GetVarValue(PlayObject, QuestActionInfo.sParam7, nTime);
    if (nTime < 0) then begin
      ScriptActionError(PlayObject, '', QuestActionInfo, sSC_SCREENEFFECT);
      Exit;
    end;
  ...
  if QuestActionInfo.sParam8 = '1' then begin
    for I := 0 to UserEngine.m_PlayObjectList.Count - 1 do begin
      ...
      if (not OnlineObject.m_boGhost) and (not OnlineObject.m_boNotOnlineAddExp) then
        OnlineObject.SendMsg(OnlineObject, RM_SCREENEFFECT, MakeWord(nPlayCount, nImageCount), MakeLong(nX, nY), nStartImage, nImageIndex, IntToStr(nTime));
    end;
  {end else
    if QuestActionInfo.sParam8 = '1' then begin
    PlayObject.SendRefMsg(RM_PLAYEFFECTEX, MakeWord(nPlayCount, nImageCount), MakeLong(nX, nY), nStartImage, nImageIndex, ''); }
  end else
    PlayObject.SendMsg(PlayObject, RM_SCREENEFFECT, MakeWord(nPlayCount, nImageCount), MakeLong(nX, nY), nStartImage, nImageIndex, IntToStr(nTime));
end;

procedure TNormNpc.ActionOfChangeSpeed(BaseObject: TActorObject; QuestActionInfo: pTQuestActionInfo);
var
  nType: Integer;
  nValue: Integer;
  nSpeed: Integer;
...
begin
  nType := Str_ToInt(QuestActionInfo.sParam1, 0);
  if not (nType in [1..3]) then begin
    ScriptActionError(BaseObject, '', QuestActionInfo, sSC_CHANGESPEED);
    Exit;
  end;
  nValue := Str_ToInt(QuestActionInfo.sParam2, 0);
  if BaseObject.m_btRaceServer = RC_PLAYOBJECT then
    GetVarValue(TPlayObject(BaseObject), QuestActionInfo.sParam2, nValue);

  nTime := Str_ToInt(QuestActionInfo.sParam3, -1);
  if BaseObject.m_btRaceServer = RC_PLAYOBJECT then
    GetVarValue(TPlayObject(BaseObject), QuestActionInfo.sParam3, nTime);


  if (nValue < -10) or (nValue > 10) then begin
    ScriptActionError(BaseObject, '', QuestActionInfo, sSC_CHANGESPEED);
    Exit;
  end;

  if nTime < 0 then nTime := 0;

  if nTime > 0 then
    BaseObject.m_ChangeSpeedTimeTick[nType - 1] := GetTickCount + nTime * 1000
  else
    BaseObject.m_ChangeSpeedTimeTick[nType - 1] := 0;

  case nType of
    1: begin
        nSpeed := BaseObject.m_nMoveSpeed;
        BaseObject.m_nMoveSpeed := nValue;
        if nSpeed <> BaseObject.m_nMoveSpeed then
          BaseObject.RefGameSpeed;
      end;
    2: begin
        nSpeed := BaseObject.m_nAttackSpeed;
        BaseObject.m_nAttackSpeed := nValue;
        if nSpeed <> BaseObject.m_nAttackSpeed then
          BaseObject.RefGameSpeed;
      end;
    3: begin
        nSpeed := BaseObject.m_nSpellSpeed;
        BaseObject.m_nSpellSpeed := nValue;
        if nSpeed <> BaseObject.m_nSpellSpeed then
          BaseObject.RefGameSpeed;
      end;
  end;
end;

procedure TNormNpc.ActionOfRecallSelf(BaseObject: TActorObject;
  QuestActionInfo: pTQuestActionInfo);
var
  sMonName: string;
  Cert: TCopyObject;
  n1C, n20, n24, nX, nY: Integer;
  p28: Pointer;
...
begin
  ...
  for n1C := 0 to 7 do begin
    if BaseObject.m_PEnvir.GetNextPosition(BaseObject.m_nCurrX, BaseObject.m_nCurrY, n1C, 1, nX, nY) then begin
  ...
  Cert.m_sMapName := BaseObject.m_sMapName;
  Cert.m_PEnvir := BaseObject.m_PEnvir;
  Cert.m_nCurrX := nX;
  Cert.m_nCurrY := nY;
  ...
  Cert.m_Abil := BaseObject.m_Abil;
  Cert.m_Abil.HP := BaseObject.m_Abil.MaxHP;
  Cert.m_Abil.MP := BaseObject.m_Abil.MaxMP;

  Cert.m_btJob := BaseObject.m_btJob;
  Cert.m_btGender := BaseObject.m_btGender;
  Cert.m_btHair := BaseObject.m_btHair;
  Cert.m_btRaceImg := BaseObject.m_btRaceImg;
  ...
  if Cert <> nil then begin
    for I := 0 to Length(BaseObject.m_UseItems) - 1 do begin
      ...
      if UserEngine.CopyToUserItemFromName(StdItem.Name, UserItem) then begin
        UserItem^ := BaseObject.m_UseItems[I];
        UserItem.btValue[13] := 0;
        Cert.m_UseItems[I] := UserItem^;
      end;
      Dispose(UserItem);
    end;

    for I := 0 to BaseObject.m_MagicList.Count - 1 do begin //添加魔法
      UserMagic := BaseObject.m_MagicList.Items[I];
      New(Magic);
      Magic^ := UserMagic^;
      Magic.btKey := VK_F1;
      Cert.m_MagicList.Add(Magic);
    end;
    Cert.m_boButchItemEx := False;
    Cert.m_boDropBagItemConfig := False;
    Cert.m_boButchItem := False;
    Cert.m_boNoDropItem := True;
    Cert.m_boNoDropUseItem := True;
    Cert.m_boNoDropUseItemConfig := True;
    Cert.RecalcAbilitys;
    ...
    if MonGen <> nil then begin
      ...
      Cert.m_PEnvir.DeleteFromMap(BaseObject.m_nCurrX, BaseObject.m_nCurrY, Cert);
      FreeAndNil(Cert);
    end;
  end;
end;

procedure TNormNpc.ActionOfCreateGroupFail(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
begin
...
procedure TNormNpc.ActionOfRangeGoto(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  I, nRange: Integer;
  BaseObjectList: TList;
  ...
  boDeath: Boolean;
begin
  nRange := Str_ToInt(QuestActionInfo.sParam3, -1);
  if nRange < 0 then begin
    ScriptActionError(BaseObject, '', QuestActionInfo, sSC_RANGEGOTO);
    Exit;
  end;
  sLabel := QuestActionInfo.sParam2;
  GetVarValue(PlayObject, QuestActionInfo.sParam2, sLabel);
  if sLabel = '' then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_RANGEGOTO);
    Exit;
  end;
  boDeath := QuestActionInfo.sParam3 = '1';
  BaseObjectList := TList.Create;
  PlayObject.GetMapActorObjects(PlayObject.m_PEnvir, PlayObject.m_nCurrX, PlayObject.m_nCurrY, nRange, BaseObjectList);
  for I := 0 to BaseObjectList.Count - 1 do begin
    BaseObject := TActorObject(BaseObjectList.Items[I]);
    if BaseObject.m_boGhost then Continue;
    if boDeath and BaseObject.m_boDeath then Continue;
    if BaseObject.m_btRaceServer = RC_PLAYOBJECT then begin
      TPlayObject(BaseObject).m_nScriptGotoCount := 0;
      GotoLable(TPlayObject(BaseObject), sLabel, False);
    end;
  end;
  BaseObjectList.Free;
end;

procedure TNormNpc.ActionOfCopyHumUseItems(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  sCharName: string;
  TargeTPlayObject: TPlayObject;
  I: Integer;
  UserItem: pTUserItem;
  StdItem: pTStdItem;
  boOK: Boolean;
begin
  ...
  if sCharName = '' then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_COPYHUMUSEITEMS);
    Exit;
  end;
  TargeTPlayObject := UserEngine.GetPlayObject(sCharName);
  if TargeTPlayObject = nil then Exit;

  PlayObject.m_Abil := TargeTPlayObject.m_Abil;
  PlayObject.m_Abil.HP := TargeTPlayObject.m_Abil.MaxHP;
  PlayObject.m_Abil.MP := TargeTPlayObject.m_Abil.MaxMP;
  PlayObject.m_WAbil := TargeTPlayObject.m_Abil;
  PlayObject.m_btJob := TargeTPlayObject.m_btJob;
  PlayObject.m_btGender := TargeTPlayObject.m_btGender;
  //PlayObject.m_btHair := TargeTPlayObject.m_btHair;
  //PlayObject.m_btRaceImg := TargeTPlayObject.m_btRaceImg;
  //if TargeTPlayObject.m_btJob <> PlayObject.m_btJob then Exit;
  boOK := False;
  for I := 0 to Length(PlayObject.m_UseItems) - 1 do begin
    if (PlayObject.m_UseItems[I].wIndex <= 0) and (TargeTPlayObject.m_UseItems[I].wIndex > 0) then begin
      StdItem := UserEngine.GetStdItem(TargeTPlayObject.m_UseItems[I].wIndex);
      if StdItem = nil then Continue;
      New(UserItem);
      if UserEngine.CopyToUserItemFromName(StdItem.Name, UserItem) then begin
        UserItem^ := TargeTPlayObject.m_UseItems[I];
        UserItem.btValue[13] := 0;
...
procedure TNormNpc.ActionOfChangeModeEx(BaseObject: TActorObject;
  QuestActionInfo: pTQuestActionInfo);
var
...
begin
  ...
  if PlayObject <> nil then begin
    if (nMode < 0) then GetVarValue(PlayObject, QuestActionInfo.sParam1, nMode); //增加变量支持
    if (nTime <= 0) then GetVarValue(PlayObject, QuestActionInfo.sParam2, nTime); //增加变量支持
    if (nValue < 0) then GetVarValue(PlayObject, QuestActionInfo.sParam3, nValue); //增加变量支持
  end;

  if (not (nMode in [1..10])) or (nTime <= 0) or ((nMode in [3..8]) and (nValue < 0)) then begin
    ScriptActionError(PlayObject, '', QuestActionInfo, sSC_CHANGEMODEEX);
    Exit;
  end;
  Dec(nMode);
  BaseObject.m_dwChangeModeExTick[nMode] := GetTickCount + nTime * 1000;
  BaseObject.m_wChangeModeExValue[nMode] := nValue;
  if nMode in [1..7] then begin
    BaseObject.RecalcAbilitys;
    if (nMode in [2..7]) and (BaseObject.m_btRaceServer in [RC_PLAYOBJECT, RC_HEROOBJECT]) then
      BaseObject.SendMsg(BaseObject, RM_ABILITY, 0, 0, 0, 0, '');
  end else
    if nMode >= 8 then begin
    BaseObject.m_boCanHit := False;
    BaseObject.m_boCanSpell := False;
    if nMode >= 9 then begin
      BaseObject.m_boCanDeal := False;
      BaseObject.m_boCanDrop := False;
      BaseObject.m_boCanUseItem := False;
      BaseObject.m_boCanWalk := False;
      BaseObject.m_boCanRun := False;
    end;
  end;
  if (nMode = 1) and (m_dwChangeModeExTick[1] > 0) then begin
    BaseObject.SendRefMsg(RM_DISAPPEAR, 0, 0, 0, 0, '');
  end;
end;

procedure TNormNpc.ActionOfSetCheckUsePlugIn(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
begin
  PlayObject.m_boCheckActionCount := QuestActionInfo.sParam1 = '1';
end;

procedure TNormNpc.ActionOfMoveItemValueFinish(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  I: Integer;
  boSourceItem, boDestItem: Boolean;
  UserItem: pTUserItem;
begin
  if PlayObject.m_boMoveItemValue and (PlayObject.m_SourceItem <> nil) and (PlayObject.m_DestItem <> nil) then begin
    boSourceItem := False;
    boDestItem := False;
    for I := 0 to PlayObject.m_ItemList.Count - 1 do begin
      UserItem := PlayObject.m_ItemList.Items[I];
      if UserItem = PlayObject.m_SourceItem then begin
        boSourceItem := True;
        break;
      end;
    end;
    for I := 0 to PlayObject.m_ItemList.Count - 1 do begin
      UserItem := PlayObject.m_ItemList.Items[I];
      if UserItem = PlayObject.m_DestItem then begin
        boDestItem := True;
        break;
      end;
    end;
    PlayObject.SendDefMessage(SM_MOVEITEMVALUE_OK, 0, 0, 0, 0, '');

    if boSourceItem then
      PlayObject.SendUpDateItem(PlayObject.m_SourceItem);
    if boDestItem then
      PlayObject.SendUpDateItem(PlayObject.m_DestItem);
  end;
end;

procedure TNormNpc.ActionOfSetMoveItemValue(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);

  procedure SetItemValue(Value: PInteger; nWhere: Integer; boSource: Boolean);
  var
    SourceByte, DestByte: PByte;
  begin
    SourceByte := nil;
    DestByte := nil;
    case nWhere of
      0: begin
          SourceByte := PlayObject.m_SourceLoBytes[0];
          DestByte := PlayObject.m_DestLoBytes[0];
        end;
      1: begin
          SourceByte := PlayObject.m_SourceLoBytes[1];
          DestByte := PlayObject.m_DestLoBytes[1];
        end;
      2: begin
          SourceByte := PlayObject.m_SourceLoBytes[2];
          DestByte := PlayObject.m_DestLoBytes[2];
        end;
      3: begin
          SourceByte := PlayObject.m_SourceLoBytes[3];
          DestByte := PlayObject.m_DestLoBytes[3];
        end;
      4: begin
          SourceByte := PlayObject.m_SourceLoBytes[4];
          DestByte := PlayObject.m_DestLoBytes[4];
        end;


      5: begin
          SourceByte := PlayObject.m_SourceHiBytes[0];
          DestByte := PlayObject.m_DestHiBytes[0];
        end;
      6: begin
          SourceByte := PlayObject.m_SourceHiBytes[1];
          DestByte := PlayObject.m_DestHiBytes[1];
        end;
      7: begin
          SourceByte := PlayObject.m_SourceHiBytes[2];
          DestByte := PlayObject.m_DestHiBytes[2];
        end;
      8: begin
          SourceByte := PlayObject.m_SourceHiBytes[3];
          DestByte := PlayObject.m_DestHiBytes[3];
        end;
      9: begin
          SourceByte := PlayObject.m_SourceHiBytes[4];
          DestByte := PlayObject.m_DestHiBytes[4];
        end;

      10: begin
          SourceByte := PlayObject.m_SourceAyomTypes[0];
          DestByte := PlayObject.m_DestAyomTypes[0];
        end;
      11: begin
          SourceByte := PlayObject.m_SourceAyomTypes[1];
          DestByte := PlayObject.m_DestAyomTypes[1];
        end;
      12: begin
          SourceByte := PlayObject.m_SourceAyomTypes[2];
          DestByte := PlayObject.m_DestAyomTypes[2];
        end;

      13: begin
          SourceByte := PlayObject.m_SourceAyomValues[0];
          DestByte := PlayObject.m_DestAyomValues[0];
        end;
      14: begin
          SourceByte := PlayObject.m_SourceAyomValues[1];
          DestByte := PlayObject.m_DestAyomValues[1];
        end;
      15: begin
          SourceByte := PlayObject.m_SourceAyomValues[2];
          DestByte := PlayObject.m_DestAyomValues[2];
        end;

      16: begin
          SourceByte := PlayObject.m_SourceItemLight;
          DestByte := PlayObject.m_DestItemLight;
        end;
      17: begin
          SourceByte := PlayObject.m_SourceStarCount;
          DestByte := PlayObject.m_DestStarCount;
        end;
    else begin
        SourceByte := PlayObject.m_SourceAddPoint[nWhere - 18];
        DestByte := PlayObject.m_DestAddPoint[nWhere - 18];
      end;
    end;
    if (SourceByte <> nil) and (DestByte <> nil) then begin
      if boSource then
        Value^ := SourceByte^
      else
        Value^ := DestByte^;
...
begin
  if PlayObject.m_boMoveItemValue and (PlayObject.m_SourceItem <> nil) and (PlayObject.m_DestItem <> nil) then begin
    ...
    if boSourceItem and boDestItem then begin
      nIndex := GetValNameNo(QuestActionInfo.sParam1);
      if nIndex >= 0 then begin
        case nIndex of
          0..999: begin
              nWhere := 0;
              nCount := _MIN(nIndex + 31, High(g_Config.GlobalVal));
              for I := nIndex to nCount do begin
                SetItemValue(@g_Config.GlobalVal[I], nWhere, QuestActionInfo.sParam2 = '1');
                Inc(nWhere);
              end;
            end;
          1000..1099: begin
              nWhere := 0;
              nIndex := nIndex - 1000;
              nCount := _MIN(nIndex + 31, High(g_Config.GlobaDyMval));
              for I := nIndex to nCount do begin
                SetItemValue(@g_Config.GlobaDyMval[I], nWhere, QuestActionInfo.sParam2 = '1');
                Inc(nWhere);
              end;
            end;
      {1100..1109: begin
      ...
      1110..1119: begin
          ...
          1200..1299: begin
              nWhere := 0;
              nIndex := nIndex - 1200;
              nCount := _MIN(nIndex + 31, High(PlayObject.m_nMval));
              for I := nIndex to nCount do begin
                SetItemValue(@PlayObject.m_nMval[I], nWhere, QuestActionInfo.sParam2 = '1');
                Inc(nWhere);
              end;
            end;
          1300..1399: begin
              nWhere := 0;
              nIndex := nIndex - 1300;
              nCount := _MIN(nIndex + 31, High(PlayObject.m_nInteger));
              for I := nIndex to nCount do begin
                SetItemValue(@PlayObject.m_nInteger[I], nWhere, QuestActionInfo.sParam2 = '1');
                Inc(nWhere);
              end;
            end;
        end;
      ...
      PlayObject.SendDefMessage(SM_STOPMOVEITEMVALUE, 0, 0, 0, 0, '');
      PlayObject.SysMsg('没有发现物品！！！', c_Red, t_Hint);
    end;
  end;
end;

procedure TNormNpc.ActionOfMoveItemValue(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  I, nWhere, nValue: Integer;
  SourceByte, DestByte: PByte;
...
//10攻击元素属性   1火  2冰  3电  4风  5神圣  6暗黑  7幻影
//11强元素属性   1火  2冰  3电  4风  5神圣  6暗黑  7幻影
//12弱元素属性   1火  2冰  3电  4风  5神圣  6暗黑  7幻影
...
//19 属性类型 (1=物理伤害减少 2=魔法伤害减少 3=忽视目标防御 4=所有伤害反射 5=增加攻击伤害)
//20 属性值
...
//31 攻击加速
  ...
  if PlayObject.m_boMoveItemValue and (PlayObject.m_SourceItem <> nil) and (PlayObject.m_DestItem <> nil) then begin
    ...
    if boSourceItem and boDestItem then begin
      ...
      if nWhere < 0 then GetVarValue(PlayObject, QuestActionInfo.sParam1, nWhere); //增加变量支持
      if nValue < 0 then GetVarValue(PlayObject, QuestActionInfo.sParam2, nValue); //增加变量支持
      if (nWhere < 0) or (nWhere > 31) or (not (nValue in [0..255])) then begin
      ...
      if (SourceByte <> nil) and (DestByte <> nil) then begin
        nValue := _MIN(nValue, SourceByte^);
        SourceByte^ := _MAX(SourceByte^ - nValue, 0);
        DestByte^ := _MIN(DestByte^ + nValue, 255);
      end;
    end else begin
      PlayObject.SendDefMessage(SM_STOPMOVEITEMVALUE, 0, 0, 0, 0, '');
      PlayObject.SysMsg('没有发现物品！！！', c_Red, t_Hint);
...
procedure TNormNpc.ActionOfUnBindUseItem(PlayObject: TPlayObject;
  QuestActionInfo: pTQuestActionInfo);
var
  UserItem: pTUserItem;
  StdItem: pTStdItem;
...
procedure TNormNpc.ActionOfChangeItemColor(BaseObject: TActorObject;
  QuestActionInfo: pTQuestActionInfo);
var
  nValue, nOValue, nWhere: Integer;
  UserItem: pTUserItem;
...
begin
  ...
  if (PlayObject <> nil) then begin
    if not (nValue in [0..255]) then
      GetVarValue(PlayObject, QuestActionInfo.sParam1, nValue); //增加变量支持
    if not (nWhere in [Low(THumanUseItems)..High(THumanUseItems)]) then
      GetVarValue(PlayObject, QuestActionInfo.sParam2, nWhere); //增加变量支持
  end;

  if not (nValue in [0..255]) then begin
    PlayObject.SendChangeItemFail;
    ScriptActionError(BaseObject, '', QuestActionInfo, sSC_CHANGEITEMCOLOR);
    Exit;
  end;
  ...
  if UserItem <> nil then begin
    nOValue := UserItem.btColor;
    UserItem.btColor := nValue;
    if nOValue <> UserItem.btColor then
      if (nWhere in [Low(THumanUseItems)..High(THumanUseItems)]) then begin
        if BaseObject.m_btRaceServer = RC_PLAYOBJECT then begin
          TPlayObject(BaseObject).SendUpdateItem(UserItem)
        end else
          if BaseObject.m_btRaceServer = RC_HEROOBJECT then begin
          THeroObject(BaseObject).SendUpdateItem(UserItem)
        end;
      end else begin
...
procedure TNormNpc.ActionOfChangeItemNewAddValue(BaseObject: TActorObject;
  QuestActionInfo: pTQuestActionInfo);
var
  cMethod: Char;
  nValue, nOValue, nValueWhere, nWhere: Integer;
  UserItem: pTUserItem;
  StdItem: pTStdItem;
  PlayObject: TPlayObject;
begin
  ...
  nWhere := Str_ToInt(QuestActionInfo.sParam4, -1);
  if (PlayObject <> nil) and (nWhere < 0) then
    GetVarValue(PlayObject, QuestActionInfo.sParam4, nWhere); //增加变量支持
  ...
  if UserItem <> nil then begin
    nValueWhere := Str_ToInt(QuestActionInfo.sParam1, -1);

    if QuestActionInfo.sParam2 <> '' then
      cMethod := QuestActionInfo.sParam2[1];

    nValue := Str_ToInt(QuestActionInfo.sParam3, -1);
    if (PlayObject <> nil) and (nValueWhere < 0) then
      GetVarValue(PlayObject, QuestActionInfo.sParam1, nValueWhere); //增加变量支持

    if (PlayObject <> nil) and (nValue < 0) then
      GetVarValue(PlayObject, QuestActionInfo.sParam3, nValue); //增加变量支持

    if (nValueWhere <= 0) or (nValueWhere > 17) or (not (nValue in [0..255])) then begin
      if (PlayObject <> nil) and (not (nWhere in [Low(THumanUseItems)..High(THumanUseItems)])) then
        PlayObject.SendChangeItemFail;
      ScriptActionError(BaseObject, '', QuestActionInfo, sSC_CHANGEITEMNEWADDVALUE);
      Exit;
    end;

    if nValueWhere <= 5 then begin
      UserItem.AddPoint[1] := nValueWhere;
      case cMethod of
      ...
      if nValueWhere < 17 then begin
        case cMethod of
          '+': UserItem.AddPoint[nValueWhere - 3] := UserItem.AddPoint[nValueWhere - 3] + nValue;
          '-': UserItem.AddPoint[nValueWhere - 3] := UserItem.AddPoint[nValueWhere - 3] - nValue;
          '=': UserItem.AddPoint[nValueWhere - 3] := nValue;
        end;
      end else begin
        case cMethod of
          '+': UserItem.AddPoint[0] := UserItem.AddPoint[0] + nValue;
          '-': UserItem.AddPoint[0] := UserItem.AddPoint[0] - nValue;
          '=': UserItem.AddPoint[0] := nValue;
        end;
      end;
    end;
...
function TNormNpc.ConditionOfCheckMapAIHumanCount(BaseObject: TActorObject;
  QuestConditionInfo: pTQuestConditionInfo): Boolean;
var
...
begin
  ...
  nHumanCount := UserEngine.GetMapHuman(Envir, True);
  cMethod := QuestConditionInfo.sParam2[1];
  case cMethod of
