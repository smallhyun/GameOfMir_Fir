...
begin
  ...
  if QusList.Count > 0 then begin
    ...
    try
      iQusID := -1;
      if (LocID >= 0) and (LocID < AnsList.Count) then
      ...
      if RandList.Count > 0 then begin
        i := Random(RandList.Count);
        if (i >= 0) and (i < RandList.Count) then
          iQusID := StrToInt(RandList[i]);
      end;
      if (iQusID >= 0) and (iQusID < QusList.Count) then
      begin
...
begin
  ...
  begin
    ...
    if (iRnd >= 0) and (iRnd < QusList.Count) then begin
      Result := QusList[iRnd];
      iAIPt := iRnd;
    end;
  end;
...
begin
  ...
  begin
    ...
    try
      ...
      i := 0;
      bHas := False;
      while i < sNowAnsList.Count - 1 do
      begin
      ...
      if (iAIPt >= 0) and (iAIPt < AnsList.Count) then
