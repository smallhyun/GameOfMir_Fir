...
type
  ...
  public
    ...
    function GetMapHuman(sMapName: string; boIsAI: Boolean = False): Integer; overload;
    function GetMapHuman(Envir: TEnvirnoment; boIsAI: Boolean = False): Integer; overload;
...
begin
  ...
  try
    ...
    MainOutMessage(sExceptionMsg8 + ' ' + IntToStr(nError));
  end;
...
begin
  for I := 0 to m_PlayObjectList.Count - 1 do begin
    PlayObject := TPlayObject(m_PlayObjectList.Objects[I]);
    if (PlayObject <> nil) and (not PlayObject.m_boGhost) and (not PlayObject.m_boNotOnlineAddExp) and (not PlayObject.m_boAI) then
...
var
  sMsg: string;
  NewBuff: array[0..DATA_BUFSIZE2 - 1] of AnsiChar;
  sDefMsg: string;
resourcestring
    ...
    case DefMsg.Ident of
      CM_SPELL: begin //3017
          ...
          if g_Config.boSpellSendUpdateMsg then begin //使用UpdateMsg 可以防止消息队列里有多个操作
            PlayObject.SendUpdateMsgA(PlayObject,
              DefMsg.Ident,
              DefMsg.Tag,
              LoWord(DefMsg.Recog),
              HiWord(DefMsg.Recog),
      ...
      CM_POWERHIT,
        ...
        CM_103HIT: begin
          if g_Config.boActionSendActionMsg then begin //使用UpdateMsg 可以防止消息队列里有多个操作
            PlayObject.SendUpdateMsgA(PlayObject, //PlayObject.SendActionMsg PlayObject.SendUpdateMsgA
              DefMsg.Ident,
              DefMsg.Tag,
...
function TUserEngine.GetMapHuman(sMapName: string; boIsAI: Boolean): Integer;
var
  I: Integer;
  Envir: TEnvirnoment;
  PlayObject: TPlayObject;
  ...
  for I := 0 to m_PlayObjectList.Count - 1 do begin
    ...
    if PlayObject <> nil then begin
      if not PlayObject.m_boDeath and not PlayObject.m_boGhost and
        ...
        if boIsAI then begin
          if PlayObject.m_boAI then
            Inc(Result);
        end else
          Inc(Result);
    end;
...
function TUserEngine.GetMapHuman(Envir: TEnvirnoment; boIsAI: Boolean): Integer;
var
  I: Integer;
  PlayObject: TPlayObject;
begin
...
begin
  try
    ...
    MainOutMessage(sExceptionMsg + ' ' + IntToStr(nError));
  end;
end;
