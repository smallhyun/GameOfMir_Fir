...
type
  TfrmViewList = class(TForm)
    ...
    ListBoxAllowHumPickUpItem: TListBox;
    GroupBox31: TGroupBox;
    ListBoxitemList8: TListBox;
    ButtonPickUpHumAdd: TButton;
    ButtonPickUpHumDel: TButton;
    ButtonPickUpHumAddAll: TButton;
    ButtonPickUpHumDelAll: TButton;
    ButtonPickUpHumSave: TButton;
    ...
    procedure ListBoxAllowHumPickUpItemClick(Sender: TObject);
    procedure ButtonPickUpHumAddClick(Sender: TObject);
    procedure ButtonPickUpHumDelClick(Sender: TObject);
    procedure ButtonPickUpHumAddAllClick(Sender: TObject);
    procedure ButtonPickUpHumDelAllClick(Sender: TObject);
    procedure ButtonPickUpHumSaveClick(Sender: TObject);
  private
    boOpened: Boolean;
    boModValued: Boolean;
...
begin
  ...
  ButtonPickUpHumSave.Enabled := True;
end;
...
begin
  ...
  ButtonPickUpHumSave.Enabled := False;
end;
...
begin
  ...
  try
    ...
    for I := 0 to UserEngine.StdItemList.Count - 1 do begin
      ...
      ListBoxitemList8.Items.AddObject(StdItem.Name, TObject(I));
  ...
  ListBoxAllowHumPickUpItem.Items.Clear;
  g_AllowHumPickUpItemList.Lock;
  try
    ListBoxAllowHumPickUpItem.Items.AddStrings(g_AllowHumPickUpItemList);
      {for I := 0 to g_AllowPickUpItemList.Count - 1 do begin
        //if g_AllowPickUpItemList.Strings[i][1]<> ';' then
        ListBoxAllowPickUpItem.Items.Add(g_AllowPickUpItemList.Strings[I]);
      end; }
  finally
    g_AllowHumPickUpItemList.UnLock;
  end;
...
procedure TfrmViewList.ListBoxAllowHumPickUpItemClick(Sender: TObject);
begin
  if not boOpened then Exit;
  if ListBoxAllowHumPickUpItem.ItemIndex >= 0 then
    ButtonPickUpHumDel.Enabled := True;
end;

procedure TfrmViewList.ButtonPickUpHumAddClick(Sender: TObject);
var
  I: Integer;
begin
  if ListBoxitemList8.ItemIndex >= 0 then begin
    for I := 0 to ListBoxAllowHumPickUpItem.Items.Count - 1 do begin
      if ListBoxAllowHumPickUpItem.Items.Strings[I] = ListBoxitemList8.Items.Strings[ListBoxitemList8.ItemIndex] then begin
        Application.MessageBox('此物品已在列表中！！！', '错误信息', MB_OK + MB_ICONERROR);
        Exit;
      end;
    end;
    ListBoxAllowHumPickUpItem.Items.Add(ListBoxitemList8.Items.Strings[ListBoxitemList8.ItemIndex]);
    ModValue();
  end;
end;

procedure TfrmViewList.ButtonPickUpHumDelClick(Sender: TObject);
begin
  if ListBoxAllowHumPickUpItem.ItemIndex >= 0 then begin
    ListBoxAllowHumPickUpItem.Items.Delete(ListBoxAllowHumPickUpItem.ItemIndex);
    ModValue();
  end;
  if ListBoxAllowHumPickUpItem.ItemIndex < 0 then
    ButtonPickUpHumDel.Enabled := False;
end;

procedure TfrmViewList.ButtonPickUpHumAddAllClick(Sender: TObject);
var
  I: Integer;
begin
  ListBoxAllowHumPickUpItem.Items.Clear;
  for I := 0 to ListBoxitemList8.Items.Count - 1 do begin
    ListBoxAllowHumPickUpItem.Items.Add(ListBoxitemList8.Items.Strings[I]);
  end;
  ModValue();
end;

procedure TfrmViewList.ButtonPickUpHumDelAllClick(Sender: TObject);
begin
  ListBoxAllowHumPickUpItem.Items.Clear;
  ButtonPickUpHumDelAll.Enabled := False;
  ModValue();
end;

procedure TfrmViewList.ButtonPickUpHumSaveClick(Sender: TObject);
var
  I: Integer;
  sItemIdx: string;
  sFileName: string;
begin
  sFileName := g_Config.sEnvirDir + 'AllowHumPickUpItemList.txt';
  g_AllowHumPickUpItemList.Lock;
  try
    g_AllowHumPickUpItemList.Clear;
    g_AllowHumPickUpItemList.AddStrings(ListBoxAllowHumPickUpItem.Items);
    //g_AllowPickUpItemList.Add(';允许分身捡取物品列表');
    {for I := 0 to ListBoxAllowPickUpItem.Items.Count - 1 do begin
      g_AllowPickUpItemList.Add(Trim(ListBoxAllowPickUpItem.Items.Strings[I]));
    end;}
  ...
  try
    g_AllowHumPickUpItemList.SaveToFile(sFileName);
  except
  end;
  uModValue();
end;
