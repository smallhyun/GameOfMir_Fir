...
begin
  if m_btRaceServer = RC_PLAYMOSTER then begin
    if m_nCopyHumanLevel > 0 then begin
      ...
      m_nRobotRandomAddValue := g_Config.nRobotRandomAddValue;
      m_nRobotRandomNotLimit := g_Config.nRobotRandomNotLimit;
      m_nRobotRandomNewAddValue := g_Config.nRobotRandomNewAddValue;
      m_nRobotRandomAddPoint := g_Config.nRobotRandomAddPoint;
      ...
      if FileExists(sFileName) then begin
        ...
        if ItemIni <> nil then begin
          ...
          m_nRobotRandomAddValue := ItemIni.ReadInteger('BaseInfo', 'RobotRandomAddValue', g_Config.nRobotRandomAddValue);
          m_nRobotRandomNotLimit := ItemIni.ReadInteger('BaseInfo', 'RobotRandomNotLimit', g_Config.nRobotRandomNotLimit);
          m_nRobotRandomNewAddValue := ItemIni.ReadInteger('BaseInfo', 'RobotRandomNewAddValue', g_Config.nRobotRandomNewAddValue);
          m_nRobotRandomAddPoint := ItemIni.ReadInteger('BaseInfo', 'RobotRandomAddPoint', g_Config.nRobotRandomAddPoint);
          m_nDieDropUseItemRate := ItemIni.ReadInteger('BaseInfo', 'DieDropUseItemRate', 100);
          m_boButchItemEx := ItemIni.ReadBool('BaseInfo', 'ButchItem', False);
          m_boDropBagItemConfig := ItemIni.ReadBool('BaseInfo', 'DropBagItem', m_boDropBagItemConfig);
          m_boButchItem := m_boButchItemEx;
          m_boNoDropUseItemConfig := ItemIni.ReadBool('BaseInfo', 'NoDropItem', False);

          //m_boNoDropUseItem := m_boNoDropItem;
         //MainOutMessage('Initialize ' + m_sCharName + ' m_boNoDropUseItemConfig:' + booltostr(m_boNoDropUseItemConfig));
          //MainOutMessage('1 Initialize ' + m_sCharName + ' m_boNoDropUseItem:' + booltostr(m_boNoDropUseItem) + ' m_boDropUseItem:' + booltostr(m_boDropUseItem));
            ...
            if StdItemNameArray[I] <> '' then begin
              ...
              //if CheckTakeOnItems(i, StdItem^) then begin
                ...
                if UserEngine.CopyToUserItemFromName(StdItemNameArray[I], UserItem) then begin
                  ...
                  if (Random(m_nRobotRandomAddValue {10}) = 0) then //几率控制
                    UserEngine.RandomUpgradeItem(UserItem); //生成极品装备

                  UserEngine._RandomItemLimitDay(UserItem, m_nRobotRandomNotLimit);

                  if (Random(m_nRobotRandomNewAddValue) = 0) then
                    UserEngine._RandomUpgradeItem(UserItem);

                  if (Random(m_nRobotRandomAddPoint) = 0) then
...
begin
  PlayObject := nil;
  //MainOutMessage('1 DropUseItems ' + m_sCharName + ' m_boNoDropUseItemConfig:' + booltostr(m_boNoDropUseItemConfig));
  if m_boNoDropUseItem or m_boNoDropUseItemConfig or (not m_boDropUseItem) or (m_Master <> nil) then Exit;
  //MainOutMessage('2 DropUseItems ' + m_sCharName + ' m_boNoDropUseItemConfig:' + booltostr(m_boNoDropUseItemConfig));
  ...
  try
    ...
    for I := Low(THumanUseItems) to High(THumanUseItems) do begin
      ...
      if Assigned(PlugInEngine.CheckCanDieScatterItem) then begin //死亡必爆
        ...
        if PlugInEngine.CheckCanDieScatterItem(Self, PAnsiChar(sCheckItemName), False) then begin
          boDropall := True;
        end;
      end;
      ...
      if Assigned(PlugInEngine.CheckNotCanScatterItem) then begin //禁止爆出
        sCheckItemName := StdItem.Name;
        if PlugInEngine.CheckNotCanScatterItem(Self, PAnsiChar(sCheckItemName), False) then begin
          Continue;
        end;
      end;
...
function THeroObject.IsAttackTarget(BaseObject: TActorObject): Boolean;
begin
  Result := False;
  if inherited IsAttackTarget(BaseObject) then begin
    Result := True;
    if m_boTarget and (m_TargetCret <> nil) and (m_TargetCret <> BaseObject) then begin
      Result := False;
    end;
...
//        if (i = U_RINGR) or (i = U_RINGL) then begin
        if (pSItem.Shape in [114, 160, 161, 162]) or (((I = U_WEAPON) or (I = U_RIGHTHAND)) and (pSItem.AniCount in [114, 160, 161, 162])) then begin
          if m_UseItems[I].Dura <= 0 then begin
            if g_Config.boDelDuraZeroItem then begin
...
begin
  ...
  if m_UseItems[U_DRESS].wIndex > 0 then begin
    if m_UseItems[U_DRESS].Dura <= 0 then begin
      if g_Config.boDelDuraZeroItem then begin
    ...
    if (m_UseItems[I].wIndex > 0) then begin
      if m_UseItems[I].Dura <= 0 then begin
        if g_Config.boDelDuraZeroItem then begin
          ...
          m_UseItems[I].wIndex := 0;
          bo19 := True;
          bo20 := bo20 or (I in [U_DRESS, U_WEAPON]);
        end;
        Continue;
      end;

      if (Random(8) <> 0) then Continue;
    ...
    if (UserItem <> nil) and (UserItem.MakeIndex = nItemIdx) then begin
      ...
      if CompareText(sUserItemName, sItemName) = 0 then begin
        ...
        if Assigned(PlugInEngine.CheckCanDropItem) then begin
          sCheckItemName := StdItem.Name;
          if not PlugInEngine.CheckCanDropItem(Self, PAnsiChar(sCheckItemName), True) then Break;
        end;
      ...
      if StdItem <> nil then begin
        if (m_UseItems[nWhere].DuraMax > m_UseItems[nWhere].Dura) and (StdItem.StdMode <> 43) then begin
          if Assigned(PlugInEngine.CheckCanRepairItem) then begin
            sCheckItemName := StdItem.Name;
            if not PlugInEngine.CheckCanRepairItem(m_Master, PAnsiChar(sCheckItemName), False) then Continue;
          end;
          ...
          case StdItem.StdMode of
                ...
                case StdItem.Shape of
                  ...
                  3: begin //使用次数
                      if UserItem.Dura > 0 then begin
                        ...
                        TPlayObject(m_Master).UseStdmodeFunItem(StdItem);
                        boEatOK := True;
                      end;
            ...
            31: begin //解包物品
                if StdItem.AniCount = 0 then begin
                  if not (StdItem.Shape in [13..16]) then begin
                  ...
                  if TPlayObject(m_Master).UseStdmodeFunItem(StdItem) then begin
                    if FoundUserItem(UserItem) then begin
                      m_ItemList.Delete(I);
                      Dispose(UserItem);
                      UserItem := nil;
...
begin
  if ((m_TargetCret = nil) or (GetTickCount - m_dwSearchTargetTick > 500)) and
    ...
    for I := 0 to m_VisibleActors.Count - 1 do begin
      ...
      if ActorObject <> nil then begin
        if (not ActorObject.m_boDeath) and (not ActorObject.m_boGhost) then begin
          if IsProperTarget(ActorObject) and (ActorObject.m_Master = nil) and
    ...
    if ActorObject18 <> nil then begin
      SetTargetCreat(ActorObject18);
    end else begin
      ActorObject18 := nil;
      n10 := 9999;
      for I := 0 to m_VisibleActors.Count - 1 do begin
...
begin
  ...
  if m_boNoDropUseItem or m_boNoDropUseItemConfig or (not m_boDropUseItem) then Exit;
...
//英雄爆包裹
  DelList := nil;
  if not m_boDropBagItemConfig then Exit;
...
begin
  ...
  SendDelayMsg(Self, RM_MAKEGHOST, 0, 0, 0, 0, '', 1000);
  ...
  if (m_Master <> nil) and (m_Master is TAIPlayObject) and m_boAI then begin
    ...
    if (sFileName <> '') and FileExists(sFileName) then begin
      ...
      //MainOutMessage(m_sCharName + ' Hero m_WAbil.Level:' + IntToStr(m_WAbil.Level));
          ...
          for I := 0 to TempList.Count - 1 do begin
            ...
            if StdItem <> nil then begin
              ...
              if UserEngine.CopyToUserItemFromName(sItemName, UserItem) then begin
                ...
                if Random(g_Config.nRobotRandomNewAddValue {10}) = 0 then
                  UserEngine._RandomUpgradeItem(UserItem);

                if Random(g_Config.nRobotRandomAddPoint) = 0 then //新增5属性
                  UserEngine.RandomItemAddPoint(UserItem);

                UserEngine._RandomItemLimitDay(UserItem, g_Config.nRobotRandomNotLimit);

                if Random(g_Config.nRobotRandomAddValue {10}) = 0 then
        ...
        m_boNoDropUseItemConfig := ItemIni.ReadBool('UseItems', 'NoDropUseItem', False);
...
function THeroObject.GetQuestFalgStatus(nFlag: Integer): Integer;
var
  n10, n14: Integer;
begin
  Result := 0;
