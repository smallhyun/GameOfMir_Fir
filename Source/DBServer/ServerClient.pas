...
uses
  ...
  D7ScktComp, Grobal2, Common, IDSocCli;
...
type
  ...
  private
    ...
    procedure LoadHumanRcd(Msg: pTDefaultMessage; sMsg: string);
    procedure SaveHumanRcd(Msg: pTDefaultMessage; sMsg: string);

    procedure LoadHeroRcd(Msg: pTDefaultMessage; sMsg: string);
    procedure SaveHeroRcd(Msg: pTDefaultMessage; sMsg: string);

    procedure NewHeroRcd(sMsg: string);
    procedure DeleteHeroRcd(sMsg: string);

    procedure SaveHumanRcdEx(Msg: pTDefaultMessage; sMsg: string);
...
{$IFEND}
    ...
    procedure ProcessServerPacket(Buffer: PAnsiChar; BufLen: Integer);
...
var
  nMsgLen: Integer;
  RecvBuffer: array[0..DATA_BUFSIZE - 1] of AnsiChar;
...
procedure TServerClient.ProcessServerPacket(Buffer: PAnsiChar; BufLen: Integer);
var
  bo25: Boolean;
  nC: Integer;
...
procedure TServerClient.LoadHumanRcd(Msg: pTDefaultMessage; sMsg: string);
var
  sHumName: string;
  sAccount: string;
  sHumanRCD: string;
...
begin
  ...
  if nCheckCode = 1 then begin
    Inc(g_nLoadHumCount);
    sHumanRCD := zEncodeBuffer(@HumanRCD, SizeOf(THumDataInfo));
    m_DefMsg := MakeDefaultMsg(DBR_LOADHUMANRCD, 1, 0, 0, Length(sHumanRCD));
    SendSocket(EncodeMessage(m_DefMsg) + EncodeString(sHumName) + '/' + sHumanRCD);
  end else begin
...
procedure TServerClient.SaveHumanRcd(Msg: pTDefaultMessage; sMsg: string);
var
  sChrName: string;
  sUserID: string;
  sHumanRCD: string;
...
begin
  ...
  if Length(sHumanRCD) = Msg.Series then
    zDecodeBuffer(sHumanRCD, @HumanRCD, SizeOf(THumDataInfo))
  else bo21 := True;
          ...
          try
            if g_HumCharDB.Open then begin
              ...
              if HumRecord.sChrName <> '' then
                if not g_HumCharDB.Add(@HumRecord) then nCode := 4 else boNew := True;
            end;
          finally
            g_HumCharDB.Close;
          end;
...
procedure TServerClient.LoadHeroRcd(Msg: pTDefaultMessage; sMsg: string);
var
  sHumName: string;
  sAccount: string;
  sHumanRCD: string;
...
{  if nCheckCode = 1 then begin
  ...
  if nCheckCode = 1 then begin
    ...
    sHumanRCD := zEncodeBuffer(@HumanRCD, SizeOf(THumDataInfo));
    m_DefMsg := MakeDefaultMsg(DBR_LOADHERORCD, 1, 0, 0, Length(sHumanRCD));
    SendSocket(EncodeMessage(m_DefMsg) + EncodeString(sHumName) + '/' + sHumanRCD);
  end else begin
...
procedure TServerClient.SaveHeroRcd(Msg: pTDefaultMessage; sMsg: string);
var
  sChrName: string;
  sUserID: string;
  sHumanRCD: string;
...
procedure TServerClient.SaveHumanRcdEx(Msg: pTDefaultMessage; sMsg: string);
var
  sChrName: string;
  sUserID: string;
  sHumanRCD: string;
