...
interface
uses
  Windows, Classes, SysUtils, StrUtils, SyncObjs, Forms, WinSock, D7ScktComp, IDSocCli, Grobal2, Common;
type
...
{$IFEND}
    procedure ExecGateBuffers(Buffer: PAnsiChar; BufLen: Integer);
...
procedure TSelectClient.ExecGateBuffers(Buffer: PAnsiChar; BufLen: Integer);
var
    ...
    while (True) do begin
      ...
      if s10 <> '' then begin
        ...
        case Ord(s19) of
          ...
          Ord('S'): begin
              if CompareLStr(s10, '127.0.0.', Length('127.0.0.')) then begin
                m_sGateaddr := s10;
...
begin
  ...
  if nCode = -1 then begin
    try
      g_HumCharDB.Lock;
      if g_HumCharDB.Index(sChrName) >= 0 then nCode := 2;
    finally
      g_HumCharDB.UnLock;
    end;
  end;
      ...
      if g_HumCharDB.Open then begin
        if g_HumCharDB.ChrCountOfAccount(sAccount) < 2 then begin
          ...
          if HumRecord.Header.sName <> '' then
            if not g_HumCharDB.Add(@HumRecord) then nCode := 2 else boNew := True;
        end else nCode := 3;
      end;
    finally
      g_HumCharDB.Close;
...
begin
  ...
  try
   ...
   { try
      if g_HumDataDB.OpenEx then begin
        nIndex := g_HumDataDB.Index(PAnsiChar(sChrName));
        if nIndex >= 0 then begin
...
{$IF DBSUSETHREAD = 1}
    sRouteIP := GateRouteIP(m_sGateaddr, nRoutePort); //ClientSocket.RemoteAddress   RemoteAddress
{$ELSE}
        ...
        try
          if g_HumDataDB.OpenEx then begin
            {for I := 0 to ChrList.Count - 1 do begin
              ...
              try
                if g_HumCharDB.GetBy(QuickID.nIndex, @HumRecord) then begin
                  nIndex := g_HumDataDB.Index(QuickID.sChrName);
                  if nIndex >= 0 then
                    nChrRecord := g_HumDataDB.Get(nIndex, @ChrRecord)
                  else
                    nChrRecord := -1;
                  MainOutMessage(Format('sAccount:%s sChrName:%s QuickID.nIndex:%d nSelectID:%d boIsHero:%s HumRecord.sChrName:%s HumRecord.boDeleted:%s  nIndex:%d nChrRecord:%d',
                    [QuickID.sAccount, QuickID.sChrName, QuickID.nIndex, QuickID.nSelectID, BoolToStr(QuickID.boIsHero), HumRecord.sChrName, BoolToStr(HumRecord.boDeleted), nIndex, nChrRecord]));
                end else begin
                  MainOutMessage(Format('sAccount:%s sChrName:%s QuickID.nIndex:%d nSelectID:%d boIsHero:%s',
                    [QuickID.sAccount, QuickID.sChrName, QuickID.nIndex, QuickID.nSelectID, BoolToStr(QuickID.boIsHero)]));
                end;
              except
                on E: Exception do begin
                  MainOutMessage(Format('I:%d', [I]));
