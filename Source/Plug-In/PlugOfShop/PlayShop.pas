...
procedure ClientBuyShopItem(PlayObject: TPlayObject; MsgObject: TObject; nType: Integer; pszMsg: PAnsiChar);
var
  sItemName: string;
  sLog: string;
  ShopItem: pTShopItem;
    ...
    if (g_CharObject.m_nGameGold >= nPrice) and (nPrice >= 0) then begin
      if g_CharObject.IsEnoughBag then begin
        ...
        if g_CharObject.AddItemToBag(UserItem) then begin
          ...
          g_PlugEngine.EngineOut.AddGameDataLogAPI(PAnsiChar(sLog));
