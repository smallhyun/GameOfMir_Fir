...
function EncodeBuffer(pszSource: PAnsiChar; nSrcLen: Integer): string;
procedure DecodeBuffer(pszSource: PAnsiChar; pszDest: PAnsiChar; bufsize: Integer);
...
function EncodeBuffer(pszSource: PAnsiChar; nSrcLen: Integer): string;
var
  pszDest: array[0..BUFFERSIZE - 1] of AnsiChar;
begin
...
begin
  FillChar(pszDest, SizeOf(pszDest), #0);
  g_PlugEngine.EngineOut.TEDcode_EncodeString(PAnsiChar(pszSource), @pszDest);
  Result := string(pszDest);
end;
...
begin
  FillChar(pszDest, SizeOf(pszDest), #0);
  g_PlugEngine.EngineOut.TEDcode_DeCodeString(PAnsiChar(pszSource), @pszDest);
  Result := string(pszDest);
end;
