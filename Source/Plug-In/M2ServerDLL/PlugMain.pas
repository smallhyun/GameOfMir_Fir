...
var
  ...
  CharArr: array[0..254] of AnsiChar;
  ...
  try
    ...
    if (Count = ConfigOption.nSize) and (nCRC = ConfigOption.nCrc) and (LoWord(ConfigOption.nVersion) = SOFTWARE_M2SERVER) then begin
      ...
      if (UserReg.sDomainName <> '') and (UserReg.nDomainName <> 0) and (StringCrc(UserReg.sDomainName) = UserReg.nDomainName) then begin
        ChangeCaptionText(PAnsiChar(AnsiString(UserReg.sDomainName)), Length(AnsiString(UserReg.sDomainName)));
      end else begin
        ChangeCaptionText(PAnsiChar(AnsiString(DecryptString(www_941SF_com))), Length(AnsiString(DecryptString(www_941SF_com))));
      end;
      sBuffer := EncryptString(IntToStr(Integer(@ProcessHumans)));
      g_PlugEngine.PlugOfEngine.HookProcessHumans(PAnsiChar(AnsiString(sBuffer)), Length(AnsiString(sBuffer)));
    end;
