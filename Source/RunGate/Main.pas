...
uses
  ...
  Dialogs, D7ScktComp, ExtCtrls, StdCtrls, WinSock, Grobal2, IniFiles, Menus, GateShare,
...
type
  ...
  private
    ...
    procedure SendSocket(SendBuffer: PAnsiChar; nLen: Integer);
    procedure ShowMainLogMsg();
    procedure LoadConfig();
    procedure StartService();
    procedure StopService();
    procedure RestSessionArray();
    procedure ProcReceiveBuffer(tBuffer: PAnsiChar; nMsgLen: Integer);
    procedure ProcessUserPacket(UserData: pTSendUserData);
    procedure ProcessPacket(UserData: pTSendUserData);
    procedure ProcessMakeSocketStr(nSocket, nSocketIndex: Integer; Buffer: PAnsiChar; nMsgLen: Integer);
...
procedure TFrmMain.SendSocket(SendBuffer: PAnsiChar; nLen: Integer);
begin
  if ClientSocket.Socket.Connected then
    ClientSocket.Socket.SendBuf(SendBuffer^, nLen);
end;

procedure TFrmMain.SendServerMsg(nIdent: Integer; wSocketIndex: Word; nSocket, nUserListIndex: Integer; nLen: Integer; Data: PAnsiChar);
var
  GateMsg: TMsgHeader;
...
var
  sMsg, sData, sDefMsg, sDataMsg, sDataText, sHumName, sClientPassWord, sPassWord1, sPassWord2: string;
  Buffer: PAnsiChar;
            ...
            if SessionArray[UserData.nSocketIdx].nPacketIdx = nPacketIdx then begin
              ...
              if (nDataLen >= DEFBLOCKSIZE) then begin
                if SessionArray[UserData.nSocketIdx].boStartLogon then begin
                  ...
                  GetMem(Buffer, Length(AnsiString(sData)) + 1);
                  Move(PAnsiChar(AnsiString(sData))^, Buffer^, Length(AnsiString(sData)) + 1);
                  ...
                  if boSendMsg then begin
                    ...
                    {if DefMsg.Ident = CM_SAY then begin
                      ...
                      Move(PAnsiChar(@DefMsg)^, Buffer^, SizeOf(TDefaultMessage));
    ...
    for i := 0 to AbuseList.Count - 1 do begin
      ...
      if AnsiContainsText(sMsg, sFilterText) then begin
        ...
        for nLen := 1 to Length(sFilterText) do begin
          sReplaceText[nLen] := AnsiChar(sReplaceWord[1]);
        end;
...
begin
  ...
  if Conf <> nil then begin
    ...
    g_sClientPassWord := Conf.ReadString(GateClass, 'PassWord', g_sClientPassWord);
    if g_sClientPassWord = '' then g_sClientPassWord := '123456';
...
begin
  ...
  if boGateReady then begin
    ...
    if nSockIdx < GATEMAXSESSION then begin
      SendServerMsg(GM_OPEN, nSockIdx, Socket.SocketHandle, 0, Length(Socket.RemoteAddress) + 1, PAnsiChar(Socket.RemoteAddress));
      Socket.nIndex := nSockIdx;
...
procedure TFrmMain.ProcReceiveBuffer(tBuffer: PAnsiChar; nMsgLen: Integer);
var
  nLen: Integer;
  Buff: PAnsiChar;
  pMsg: pTMsgHeader;
...
procedure TFrmMain.ProcessMakeSocketStr(nSocket, nSocketIndex: Integer; Buffer: PAnsiChar; nMsgLen: Integer);
var
  sSendMsg: string;
  ...
  try
    ...
    if nMsgLen < 0 then begin
      ...
      if (nMsgLen >= SizeOf(TDefaultMessage)) then begin
        ...
        if nMsgLen > SizeOf(TDefaultMessage) then begin
          sSendMsg := '#' + EncodeMessage(pDefMsg^) + string(PAnsiChar(@Buffer[SizeOf(TDefaultMessage)])) + '!';
        end else begin
...
begin
  ...
  with frmGeneralConfig do begin
    ...
    EditClientPassword.Text := g_sClientPassWord;
  end;
...
begin
  ...
  for i := 0 to TempBlockIPList.Count - 1 do begin
    frmIPaddrFilter.ListBoxTempList.Items.Add(string(inet_ntoa(TInAddr(pTSockaddr(TempBlockIPList.Items[i]).nIPaddr))));
  end;
  for i := 0 to BlockIPList.Count - 1 do begin
    frmIPaddrFilter.ListBoxBlockList.Items.Add(string(inet_ntoa(TInAddr(pTSockaddr(BlockIPList.Items[i]).nIPaddr))));
  end;
