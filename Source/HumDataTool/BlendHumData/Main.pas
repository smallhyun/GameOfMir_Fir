 ...
 { for I := 0 to m_ItemDblClickListB.Count - 1 do begin
  ...
  if m_DynamicVarListA.Count = m_DynamicVarListB.Count then begin
    ...
    for I := 0 to m_DynamicVarListB.Count - 1 do begin
      ...
      MemoryIniFileA.Sections.AddStrings(MemoryIniFileB.Sections);
...
//------------------------------------------------------------------------------
  ...
  for I := 0 to m_DynamicVarListA.Count - 1 do begin
    MemoryIniFileA := TMemoryIniFile(m_DynamicVarListA.Objects[I]);
    //MemoryIniFileA.Sections.SaveToFile('MemoryIniFileA.txt');
    for II := MemoryIniFileA.Sections.Count - 1 downto 0 do begin
      if m_FileDBA.m_MirCharNameList.GetIndex(MemoryIniFileA.Sections.Strings[II]) < 0 then begin
        MemoryIniFileA.Sections.Objects[II].Free;
        MemoryIniFileA.Sections.Delete(II);
      end;
    end;
  end;
end;
