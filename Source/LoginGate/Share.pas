...
interface
uses
  Windows, Messages, Classes, SysUtils, D7ScktComp, Mudutil, IniFiles, HUtil32, Forms,
  Grobal2, GateShare, HttpGet;
resourcestring
  g_sUpDateTime = '更新日期: 2012/07/15';
...
begin
  ...
  if (GetTickCount - m_dwKeepAliveTick) > 12 * 1000 then begin
...
var
  nMsgLen: Integer;
  nCheckCode: Integer;
  ReceiveBuffer: array[0..DATA_BUFSIZE * 2 - 1] of AnsiChar; //array[0..4095] of PAnsiChar;
  sReviceBuff: AnsiString;
  sSocketMsg: string;
    ...
    if g_boServiceStart then begin
      if not ClientSocket.Socket.Connected then begin
        ...
        if GetTickCount - m_dwConnTick > 1000 then begin
          ...
          nCheckCode := 1;
          ClientSocket.Active := False;
          if g_Config.ServerAddr = '127.0.0.2' then
            ClientSocket.Address := '127.0.0.1'
          else
            ClientSocket.Address := g_Config.ServerAddr;
          ClientSocket.Port := g_Config.ServerPort;
          //MainOutMessage('Execute 1 ' + g_Config.ServerAddr, nil);
          try
            ClientSocket.Active := True;
          except
            Sleep(1000);
            //MainOutMessage('Execute 2 ' + g_Config.ServerAddr, nil);
            Continue;
          end;
          nCheckCode := 2;
        end;
...
begin
  if ClientSocket.Socket.Connected and (not m_boConnected) then begin
    ...
    Initialize();
    SendText('%S' + g_Config.ServerAddr + '$');
