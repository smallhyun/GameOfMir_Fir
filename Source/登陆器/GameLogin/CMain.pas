  ...
  if g_CurrentScene = s_Play then begin
    if g_PlayScene.m_MySelf <> nil then begin
      if GetTickCount - m_dwRunOneTick >= 1000 then begin
        ...
        StatusBar.Panels[1].Text := Format('地图:%s(%s) 座标:(%d:%d)', [g_sMapTitle, g_sMapName, g_PlayScene.m_MySelf.m_nCurrX, g_PlayScene.m_MySelf.m_nCurrY]);
      end;
    end;
  end;
end;
  ...
  if Length(rstr) >= 2 then begin
    if (rstr[1] = '@') and (rstr[2] = '@') then begin
      ...
      if rstr = '@@buildguildnow' then begin
        g_MessageDlg.MessageDlg('请输入行会名称:', [mbOK]);
      end else begin
        g_MessageDlg.MessageDlg('输入信息:', [mbOK]);
      end;
