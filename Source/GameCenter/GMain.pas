...
uses
  ...
  VCLUnZip, VCLZip, Common, D7ScktComp;
...
type
  TfrmMain = class(TForm)
    ...
    CheckBoxDoubleLineMode: TCheckBox;
    ...
    procedure CheckBoxDoubleLineModeClick(Sender: TObject);
...
begin
  ...
  if g_boDoubleLineMode then begin
    sExtNetComIPaddr := Trim(EditGameExtNetComIPaddr.Text);

    if (sExtNetComIPaddr = '') or not IsIPaddr(sExtNetComIPaddr) then begin
...
procedure TfrmMain.GenDBServerConfig;
  ...
  begin
    Result := 0;
    nC := 0;
    for I := 0 to Length(g_RunGateInfo) - 1 do begin
      if g_RunGateInfo[I].boGetStart then begin
        if nC = nIndex then begin
...
begin
  ...
  case nIndex of //
    ...
    1: begin
        ...
        sServerAddr := g_sSelGate_ServerAddr1;
      end;
  end;
  IniGameConf := Tinifile.Create(sIniFile + g_sSelGate_ConfigFile);
  IniGameConf.WriteString('SelGate', 'Title', g_sGameName);
...
begin
  ...
  CheckBoxDoubleLineMode.Checked := g_boDoubleLineMode;
...
begin
  ...
  if LoginGate1.boGetStart then begin
...
begin
  ...
  if LoginGate1.boGetStart and (LoginGate1.btStartStatus in [2, 3]) then begin
    GetExitCodeProcess(LoginGate1.ProcessHandle, dwExitCode);
    if dwExitCode = STILL_ACTIVE then begin
      if LoginGate1.btStartStatus = 3 then begin
        if GetTickCount - g_dwStopTick > g_dwStopTimeOut then begin
...
begin
  case wIdent of
    ...
    SG_STARTOK: begin
        if SelGate1.btStartStatus <> 2 then begin
          SelGate1.btStartStatus := 2;
        end else begin
          SelGate1.btStartStatus := 2;
        end;
...
procedure TfrmMain.CheckBoxDoubleLineModeClick(Sender: TObject);
begin
  //EditGameExtIPaddr.Enabled := not CheckBoxDoubleLineMode.Checked;
  g_boDoubleLineMode := CheckBoxDoubleLineMode.Checked;
  LabelNetComIPaddr.Visible := CheckBoxDoubleLineMode.Checked;
  EditGameExtNetComIPaddr.Visible := CheckBoxDoubleLineMode.Checked;
...
begin
  ...
  EditGameName.Text := 'MakeGM';
  EditGameExtIPaddr.Text := '127.0.0.1'; //g_sExtIPaddr;
  EditGameExtNetComIPaddr.Text := '127.0.0.2'; //g_sExtNetComIPaddr;
  CheckBoxDoubleLineMode.Checked := False;
end;
