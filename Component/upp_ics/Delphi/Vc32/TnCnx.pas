...
const
  TnCnxVersion       = 212;
  CopyRight : String = ' TTnCnx (c) 1996-2006 F. Piette V2.12 ';
...
type
  ...
  private
    ...
    function  TnCnxAllocateHWnd(Method: TWndMethod) : HWND;
    procedure TnCnxDeallocateHWnd(WHandle : Cardinal);
...
begin
    ...
    { If the pointer doesn't represent a TTnCnx, just call the default procedure}
    if not (Obj is TTnCnx) then
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
function TTnCnx.TnCnxAllocateHWnd(Method: TWndMethod) : HWND;
begin
{$IFDEF NOFORMS}
    Result := XSocketAllocateHWnd(Self);
    SetWindowLong(Result, GWL_WNDPROC, LongInt(@TnCnxWindowProc));
{$ELSE}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure TTnCnx.TnCnxDeallocateHWnd(WHandle : Cardinal);
begin
{$IFDEF NOFORMS}
...
begin
    ...
    FWindowHandle             := TnCnxAllocateHWnd(WndProc);
    FLocation                 := 'TNCNX';
    FTermType                 := 'VT100';
    FPort                     := '23';
...
begin
    ...
    TnCnxDeallocateHWnd(FWindowHandle);
    inherited Destroy;
end;
