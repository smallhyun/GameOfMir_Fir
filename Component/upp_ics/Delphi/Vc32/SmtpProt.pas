...
const
  SmtpCliVersion     = 250;
  CopyRight : String = ' SMTP component (c) 1997-2006 Francois Piette V2.50 ';
...
{$ENDIF}
  ...
  WM_SMTP_DATA_NEXT                  = WM_USER + 2;                 {AG}
{$IFDEF USE_SSL}
  WM_SMTP_QUIT_DELAYED               = WM_USER + 3;                 {AG}
{$ENDIF}
...
{$ENDIF}
        ...
        {procedure   WMSmtpData(var msg: TMessage);
                        message WM_SMTP_DATA;}
    public
...
begin
    ...
    while rPos <= Length(HdrBody) do begin
        ...
        while (Length(S) > 0) and (S[1] in [' ', #09]) do
            Delete(S, 1, 1);
...
begin
    ...
    if FBodyFlag then begin
        ...
        if FMailMsgTextPos > 0 then begin
            { This code is broken with Delphi 1 because TStrings.Text doesn't }
            { exists. It is replaced by TStrings.GetText which returs a PChar }
            {$IFNDEF DELPHI1}
            sLine := DoGetMsgTextLine(FMailMsgTextPos, FMailMessage.Text);
            {$ELSE}
            raise Exception.Create('TSmtpCli.TriggerGetData broken with Delphi 1');
            {$ENDIF}
