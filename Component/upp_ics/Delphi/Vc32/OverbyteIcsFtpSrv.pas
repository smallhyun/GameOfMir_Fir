...
const
    FtpServerVersion         = 147;
    CopyRight : String       = ' TFtpServer (c) 1998-2006 F. Piette V1.47 ';
...
{$ENDIF}
{$IFDEF USE_SSL}
    if ftpImplizitSsl in TSslFtpserver(Self).FFtpSslTypes then   { V1.47 }
        Client.CurFtpSslType := curftpImplizitSsl;               { V1.47 }
{$ENDIF}
...
{$IFNDEF NO_DEBUG_LOG}                                       { V1.46 }
    if CheckLogOptions(loProtSpecDump) then
        DebugLog(loProtSpecDump,  IntToHex(Integer(Client), 8) +
                 ' ClientConnect error ' + IntToStr(AError) + ' ' +
...
{$IFDEF STREAM64}                      { V1.43 }
               (GetFileSize64(Client.FilePath) > -1) then
{$ELSE}
...
{$ENDIF}
                ...
                Client.DataStream    := TFileStream.Create(Client.FilePath,
                                        fmCreate);
{$IFDEF STREAM64}                      { V1.43 }
            NewPos := MySeek64 (Client.DataStream, Client.RestartPos, soFromBeginning);
{$ELSE}
            NewPos := Client.DataStream.Seek(Client.RestartPos, soFrombeginning);
{$ENDIF}
...
{$IFDEF STREAM64}                      { V1.43 }
            Client.RestartPos := GetFileSize64(Client.FilePath);
{$ELSE}
...
{        if Client.DataSocket.Listen <> 0 then
        ...
        if Client.sin.sin_addr.s_addr = WSocket_htonl($7F000001) then
            ...
            if FPasvIpAddr = '' then
                ...
                PASVAddr.S_addr := WSocket_inet_addr(FPasvIpAddr);
...
begin
    ...
    {$IFDEF USE_SSL}
        if TSslFtpserver(Self).FFtpSslTypes <> [] then begin             { V1.47 }
            if not (ftpImplizitSsl in TSslFtpserver(Self).FFtpSslTypes) then begin
                Answer := Answer + '  AUTH ';
