...
{$IFDEF DELPHI1}
    ...
    {$DEFINE NO_AUTHENTICATION_SUPPORT}  { Not for Delphi 1 }
{$ENDIF}

{ DEFINE USE_ZLIB}    { Experimental code, doesn't work yet }
{ DEFINE NO_AUTHENTICATION_SUPPORT}
...
{$IFNDEF NO_DEBUG_LOG}
    IcsLogger,
{$ENDIF}
    WSocketS, IcsMD5, ExtCtrls, MimeUtil;

const
    THttpServerVersion = 140;
    CopyRight : String = ' THttpServer (c) 1999-2006 F. Piette V1.40 ';
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
procedure AuthDigestCvtHex(_b: pchar; _h: pchar);
var
    I: Word;
    J: Char;
begin
...
begin
    ...
    if FAuthType = atNone then
        FAuthenticated := TRUE
    else if FAuthType = atBasic then begin
    ...
    else if FAuthType = atDigest then begin
        FAuthDigestBody := '';
...
{$IFNDEF NO_AUTHENTICATION_SUPPORT}
    if FAuthType = atDigest then begin
        FAuthDigestServerNonce  := '';
...
{$ENDIF}
         Header := FVersion + ' 401 Unauthorized' + #13#10 +
                   'WWW-Authenticate: Basic Realm=' + FAuthRealm +
                   'Content-Type: text/html' + #13#10 +
                   'Content-Length: ' + IntToStr(Length(Body)) + #13#10 +
                   #13#10 + #13#10;
{$IFNDEF NO_AUTHENTICATION_SUPPORT}
...
{$ENDIF}
        ...
        { FState := hcRequest;      Bjornar.
          Because client might pipeline requests, FState should only be set
          to hcRequest in ConnectionDataAvailable after receiving empty line
          (when GET) and after all posted data is received (when POST). }
