...
const
    FtpCtrlSocketVersion = 145;
    CopyRight : String   = ' TFtpCtrlSocket  (c) 1998-2006 F. Piette V1.45 ';
    DefaultRcvSize       = 2048;
...
const
    ...
    ftpcMLST      = 28;   {angus Added MLST type           }
    ftpcMLSD      = 29;   {angus Added MLSD type           }
    ftpcMFMT      = 30;   {angus Added MFMT type           }
    ftpcMD5       = 31;   {angus Added MD5 type            }
{$IFNDEF USE_SSL}
    ftpcLast      = 31;   {angus used to dimension FCmdTable, note it needs to be 5 larger}
{$ELSE}
    ftpcAUTH      = 32;
    ftpcCCC       = 33;
    ftpcPBSZ      = 34;   {V1.45}
...
type
    TFtpOption    = (ftpcUNC, ftpCwdCheck, ftpCdupHome);     { angus V1.39 }
...
{$IFDEF USE_SSL}
    TCurFtpSslType  = (curftpSslNone,   curftpAuthSsl,      curftpAuthTls,
                       curftpAuthTlsP,  curftpAuthTlsC ,    curftpImplizitSsl);
{$ENDIF}
...
{$IFDEF USE_SSL}
        function    SslSendPlain(Data : Pointer; Len : Integer) : Integer;
{$ENDIF}
...
{* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *}
{$IFDEF USE_SSL}
function TFtpCtrlSocket.SslSendPlain(Data : Pointer; Len : Integer) : Integer;
begin
