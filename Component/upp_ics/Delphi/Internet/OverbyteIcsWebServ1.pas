...
Mar 04, 2006 V1.10 Added authentification demo pages.
...
const
  WebServVersion     = 109;
  CopyRight : String = 'WebServ (c) 1999-2006 F. Piette V1.09 ';
...
begin
    { It's easyer to do the cast one time. Could use with clause... }
    ClientCnx := TMyHttpConnection(Client);
    if (ClientCnx.AuthType     = atDigest) and
       (ClientCnx.AuthUserName = 'test') then
        Password := 'digest'
    else if (ClientCnx.AuthType     = atBasic) and
            (ClientCnx.AuthUserName = 'test') then
        Password := 'basic';
end;
